theory LabeledDefaultTransition
  imports "../../Final_ML"
begin
...
definition Chart_A :: state where " Chart_A = State [''A'']
  ...
  (print1 ''Exit A'' )
  []
  [Trans (P [''A'']) (S [''E_one'', ''E_two'']) (Bc True) (SKIP) (SKIP) (P [''B''])]
  (No_Composition)"
...
definition Chart_B :: state where " Chart_B = State [''B'']
  ...
  []
  []
  (Or [Trans (NONE) (S [''E_one'']) (Bc True) (SKIP) (SKIP) (P [''B'', ''B1''])
, (Trans (NONE) (S [''E_two'']) (Bc True) (SKIP) (SKIP) (P [''B'', ''B2''] )) ]
 (False) (f_Chart_B))"
