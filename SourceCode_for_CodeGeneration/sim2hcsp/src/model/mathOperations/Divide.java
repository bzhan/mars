...
public class Divide extends SL_Block {

	public Divide(String blockType, String blockName, Node node) {
	...
	public HashMap<String, String> semanticFunctionContinuous() {
		...
		if (op.startsWith("/") || op.startsWith("*")) {
			...
			for (int i = 1; i <= op.length(); i++) {
				str = str + String.valueOf(parameters.get("op").charAt(i - 1)) + getinVarNames().get(i);
			}
			str += ")";
		} else {
