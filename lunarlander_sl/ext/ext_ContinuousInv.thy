...
unfolding Valid_def
  ...
  proof-
    have 1:"big_step (Cont ode b) (a,p 0) [WaitBlk (ereal T) (\<lambda>\<tau>. EState (a,p \<tau>)) ({}, {})] (a,p T)"
      apply (rule big_step.intros)
      using pre by auto
    have 2:"(P @\<^sub>t ode_inv_assn c) (tr @ [WaitBlk (ereal T) (\<lambda>\<tau>. EState (a,p \<tau>)) ({}, {})])"
      using assms(1) 1
      unfolding Valid_def using pre by auto
    obtain tra1 tra2 where tra1:"P tra1" and tra2:"ode_inv_assn c tra2" and tra3:"tra1 @ tra2 = (tr @ [WaitBlk (ereal T) (\<lambda>\<tau>. EState (a,p \<tau>)) ({}, {})])"
      using 2 unfolding join_assn_def by auto
    obtain T' p' where Tp:"tra2 = [WaitBlk (ereal T') (\<lambda>\<tau>. EState (a,p' \<tau>)) ({}, {})]"
      using tra2 tra3 apply (induct rule: ode_inv_assn.induct)
      by auto
    have 3:"ode_inv_assn c [WaitBlk (ereal T) (\<lambda>\<tau>. EState (a,p \<tau>)) ({}, {})]"
    ...
    have 5:"big_step (Cont ode (\<lambda>s. b s \<and> c s)) (a,p 0) [WaitBlk (ereal T) (\<lambda>\<tau>. EState (a,p \<tau>)) ({}, {})] (a,p T)"
      apply (rule big_step.intros)
      using pre 4 by auto
    show ?thesis
      using assms(2) 
