...
fun enable :: "transition \<Rightarrow> string \<Rightarrow> vals \<Rightarrow> path \<Rightarrow> bool" where
   Enable1: "enable (Trans src (S strl) c a1 a2 trg) e v p = (((strl = []) \<or> (inner e strl)) 
           \<and> (cval c v p))"
 | Enable2: "enable (Trans src (T1 temp_l) c a1 a2 trg) e v p = ((tval1 temp_l v p) 
           \<and> (cval c v p))"
...
fun update_vals1 :: "exp \<Rightarrow> exp \<Rightarrow> vals \<Rightarrow> vals" where
  "update_vals1 a1 (V x) (Vals s1 h1 h2 (output1, output2)) = 
    (Vals (s1(x := (aval a1 (Vals s1 h1 h2 (output1, output2)) []))) h1 h2 (output1, output2))"
...
| Assign: "v1 = (Vals h1 h2 h3 (output1, output2))
  \<Longrightarrow> v2 = (Vals (h1(x := aval a v1 src_p)) h2 h3 (output1, output2)) 
...
| Print1: "v1 = (Vals h1 h2 h3 (output1, output2))
  \<Longrightarrow> v2 = (Vals h1 h2 h3 (output1@[a], output2))
  \<Longrightarrow> act_exec env (Print1 a) e src_p (Status v1 I) (Status v2 I) True"
| Print2: "v1 = (Vals h1 h2 h3 (output1, output2))
  \<Longrightarrow> v2 = (Vals h1 h2 h3 (output1, output2@[aval a v1 src_p]))
...
| Trans_Execution1:"enable t e v1 src_p
  \<Longrightarrow> act_exec env (condition_action t) e src_p (Status v1 I1) (Status v2 I2) True
  \<Longrightarrow> a = (transition_action t) 
  \<Longrightarrow> trg = (target t)
  \<Longrightarrow> trans_exec env t e src_p (Status v1 I1) (Status v2 I2) True a trg"

| Trans_Execution2:"enable t e v1 src_p
  \<Longrightarrow> act_exec env (condition_action t) e src_p (Status v1 I1) (Status v2 I2) False
  \<Longrightarrow> trans_exec env t e src_p (Status v1 I1) (Status v2 I2) False SKIP NONE"
...
| TLinduction: "\<not>(enable t e v1 src_p)
  \<Longrightarrow> T \<noteq> []
  \<Longrightarrow> translist_exec env T e src_p (Status v1 I1) (Status v2 I2) n b A trg p
  \<Longrightarrow> translist_exec env (t#T) e src_p (Status v1 I1) (Status v2 I2) n b A trg p"

\<comment> \<open>When the last transition fails, the transition list fails, with status 0.\<close>
| Fail: "\<not>(enable t e (get_vals s) src_p)
  \<Longrightarrow> translist_exec env [t] e src_p s s (0::int) True [] NONE []"
...
| State_Entry1:"(ST = State name a1 a2 a3 tl1 tl2 C) 
  \<Longrightarrow> v1 = (Vals h1 h2 h3 (output1, output2))
  \<Longrightarrow> v2 = (Vals h1 (h2(name := (\<lambda>str. 0::int))) (h3(name := 0 )) (output1, output2))
  \<Longrightarrow> I1 name = Info b1 str1 str2
...
| Outer_Trans_Execution1:
  "ST1 = (State name a1 a2 a3 tl1 tl2 C)
   ...
   \<Longrightarrow> v2 = (Vals h1 h2' h3' (output1, output2))
...
| Fail_Trans_Execution:
  "ST1 = (State name a1 a2 a3 tl1 tl2 C)
   ...
   \<Longrightarrow> h3' = (if (is_send = True) then h3 else h3(name := (h3 name)+1))
   \<Longrightarrow> v2 = (Vals h1 h2' h3' (output1, output2))
   \<Longrightarrow> translist_exec env tl2 e name (Status v2 I1) (Status v3 I2) n1 True A trg1 path1
...
| Start_Execution:"is_active_state [] I1 = False
