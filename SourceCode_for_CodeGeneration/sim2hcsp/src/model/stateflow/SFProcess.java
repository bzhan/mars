...
public class SFProcess {
	...
	protected static ArrayList<String> getMonitor(Stateflow sf) {
		...
		String triProcess = triger + "(num:=(Real 1);empty(EL);(addL EL E);"
				+ "empty(NL);(addL NL (Real 1)));";
		...
		for (int i = 1; i <= size; i++) {
			...
			// results.add(SFProcess.delTail(vin));
			// vin = ";P" + sf.getName() + num++;
			String monitor = "IF (num[=](Real " + i + ")) (" + "BC_" + i
					+ "!!E" + vout + " ; ((BR_" + i
					+ "??E;(addL EL E);(addL NL (Real 1));num:=(Real 1)) [[ BO_"
					+ i + "??vBO" + i + vin
		...
		String monitorE = "IF (num[=](Real " + (1 + size)
				+ ")) (delL(EL);delL(NL);IF isEmpty(EL) (" + "num:=(Real 0);E:=(String '''');"
				+ SFProcess.delTail(sending + sending) + ");"
				+ "IF([~]isEmpty(EL)) (E:=readL(EL);num:=readL(NL)))";
		...
		if (pInit.replaceAll(" ", "").isEmpty())
			monitor = "(num:=(Real 0);E:=(String '''');";
		else
			monitor = "((num:=(Real 0);E:=(String '''');" + SFProcess.delTail(pInit) + ")";
