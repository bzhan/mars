...
definition Chart_A :: state where " Chart_A = State [''A'']
  ...
  (print1 ''Exit A'' )
  []
  [Trans (P [''A'']) (S [''E_one'']) (Bc True) (SKIP) (print1 ''Transition Action1'' ) (J [''3''])]
  (No_Composition)"
...
" g = (λ str. if str = [''3''] then [Trans (J [''3'']) (S []) ((V ''x'') [>] (N (-1))) (print1 ''Condition Action2'' ) (SKIP) (P [''C'']),
Trans (J [''3'']) (S []) (Bc True) (SKIP) (SKIP) (P [''B''])] else 
[])"
...
schematic_goal "Root_Exec_for_times env [''E_one'', ''E_one''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''Entry A'', ''Condition Action2'', ''Exit A'', ''Transition Action1'', ''Entry C''], ?o2)) (?I))"
