...
public class Bias extends SL_Block {

	public Bias(String blockType, String blockName, Node node) {
		...
		parameters.clear();
		parameters.put("bias", "0");
		parameters.put("st", "0");

		for (Node n = node.getFirstChild(); n != null; n = n.getNextSibling()) {
			if (n.getNodeType() == Node.ELEMENT_NODE && n.getAttributes().getNamedItem("Name") != null
					&& n.getAttributes().getNamedItem("Name").getNodeValue().equals("Bias")) {
				parameters.put("bias", n.getFirstChild().getNodeValue());
			}
		}
	...
	public HashMap<String, String> semanticFunctionContinuous() {
		...
		String bias = "(Real " + super.getParameters().get("bias") + ")";

		ret.put("init", "");
		ret.put("0", Out1 + "=" + In1 + "+" + bias);
		ret.put("0b", "");
		return ret;
	}
