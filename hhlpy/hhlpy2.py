...
class Condition:
    ...
    def __init__(self, expr, pos, seq_labels=[], nest_label=None, annot_pos=None, categ=None, vc=False, pc = False):
        ...
        self.categ = categ
        # Whether this predicate is a verification condition or not
        self.vc = vc

        # The branch label of this verification condition, 
        # set as None when this predicate is not a verification condition.
        self.branch_label = None
        # Compute the branch label when vc is True
        if self.vc:
            if seq_labels and not nest_label:
                self.branch_label = label.SequenceLabel(*seq_labels[::-1])
        ...
        if self.vc:
            if self.categ and self.branch_label:
                ...
                self.comp_label = self.branch_label
            else:
...
class CmdVerifier:
    ...
    def compute_label(self, pos, sub_pre=None, index=None, i_num=None, value=None):
        """Compute the seq_labels and nest_label for the pre-condition.
            ...
            value: the label value of ODE, only necessary in ODE
        ...
        # When the hcsp program is ODE
            ...
            new_label = label.AtomLabel(value=value)
