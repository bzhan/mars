...
void sched_EDF(struct Thread **threads, int threadNum, int iterCount)
{
    ...
    // Deadline of the running thread
    int running_deadline = 10000;
    ...
    while (globalCount < iterCount)
    {
        ...
        for (int i = 0; i < threadNum; i++) {
            if (threads[i]->state == "READY" && threads[i]->deadline < running_deadline) {
                running_deadline = threads[i]->deadline;
                running_id = i;
            }
        }
