...
definition Chart_A :: state where " Chart_A = State [''A'']
  ...
  [Trans (P [''A'']) (S [''E_one'']) ((V ''x'') [>] (N 0)) (SKIP) (print1 ''Transition Action1'' ) (P [''B''])]
  (No_Composition)"
...
definition Chart_B :: state where " Chart_B = State [''B'']
  ...
  (print1 ''Exit B'' )
  []
  [Trans (P [''B'']) (S [''E_two'']) ((V ''x'') [>] (N 0)) (SKIP) (print1 ''Transition Action3'' ) (P [''A'']), Trans (P [''B'']) (S []) (Bc True) (SKIP) (print1 ''Transition Action4'' ) (P [''B''])]
  (No_Composition)"
...
schematic_goal "Root_Exec_for_times env [''E_one'', ''E_one''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''Entry A'', ''During A'', ''Transition Action2''], ?o2)) (?I))"
