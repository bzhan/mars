...
class Predicate:
    ...
    def __init__(self, expr, pos, path, annot_pos=None, categ=None):
...
class CmdVerifier:
    ...
    def __init__(self, *, pre, hp, post, constants=set(), z3 = True, wolfram_engine = False):
        ...
        self.infos[root_pos].post = [Predicate(post, [root_pos], [])]
    ...
    def compute_wp(self, *, pos=((),())):
        ...
        if isinstance(cur_hp, hcsp.Skip):
            # Skip: {P} skip {P}
            pre = [Predicate(vc.expr, vc.pos + [pos], vc.path, vc.annot_pos, vc.categ) for vc in post]
            ...
            pre = cur_post
            pre = [Predicate(vc.expr, vc.pos + [pos], vc.path, vc.annot_pos, vc.categ) for vc in pre]
            ...
            if not self.infos[pos].inv:
                ...
                for i, sub_inv in enumerate(cur_hp.inv):
                    ...
                    if sub_pos not in self.infos:
                        self.infos[sub_pos] = CmdInfo()
                    sub_info = self.infos[sub_pos]
                    sub_info.inv = [Predicate(sub_inv.inv, [pos], [], (i,))]
                    sub_info.assume += self.infos[pos].assume
                ...
                pre = [Predicate(expr=sub_inv.inv, pos=[pos], path=[], 
                                             annot_pos=(i,), categ="init") \
                       for i, sub_inv in enumerate(cur_hp.inv)]
            ...
            # TODO: also run if no invariants are specified? testVerify62 testVerify54 testVerify53 testVerify52 testVerify50 testVerify55
                ...
                if subposts[-1] is not expr.false_expr:
                    for vc in post:
                        ...
                        self.infos[pos].vcs.append(Predicate(e, vc.pos + [pos], vc.path, categ=vc.categ))

                # Post below is the postcondition of the invariant triple.
                post = [Predicate(subposts[-1], [pos], [])]
            ...
            # When dI_inv is set or by default
                ...
                if dI_inv != post_conj:
                    self.infos[pos].vcs.append(
                        Predicate(expr.imp(dI_inv, post_conj), [pos], []))
            ...
            #                                       {P} c {Q}
                ...
                for i, diff_cut in enumerate(diff_cuts):
                    ...
                    self.infos[sub_pos].post = [Predicate(diff_cut, [pos], [])] 
            ...
            #                   {I} <x_dot = f(x) & D> {I}
                ...
                if self.infos[pos].ghost_eqs:
                    ...
                    self.infos[sub_pos].post = [Predicate(ghost_inv, [pos], [])]
                ...
                if dG_inv != post_conj:
                    self.infos[pos].vcs.append(
                        Predicate(expr.imp(dG_inv, post_conj), [pos], []))
            ...
            # Cases when dbx_inv is e >(>=) 0.
                ...
                if self.infos[pos].dbx_inv is None:
                    ...
                    self.infos[pos].vcs.append(
                        Predicate(expr.imp(self.infos[pos].dbx_inv, post_conj), [pos], []))
            ...
            #      {e >=(>) 0} <x_dot = f(x) & D> {e >=(>) 0}
                ...
                if self.infos[pos].barrier_inv is None:
                    ...
                    self.infos[pos].vcs.append(
                        Predicate(expr.imp(self.infos[pos].barrier_inv, post_conj), [pos], []))
        ...
        if self.infos[pos].pre is None:
            ...
            for i, vc in enumerate(pre):
                ...
                if isinstance(last_hp, hcsp.Loop):
                    ...
                    annot_pos = None
                self.infos[pos].vcs.append(
                    Predicate(expr.imp(self.infos[pos].pre, vc.expr), vc.pos, vc.path, annot_pos=annot_pos, categ=vc.categ))
        ...
        if self.infos[pos].assume:
            ...
            for i in range(len(self.infos[pos].vcs)):
                ...
                self.infos[pos].vcs[i] = Predicate(expr.imp(assume, vc.expr), vc.pos, vc.path, vc.annot_pos, vc.categ)
                    ...
                    if isinstance(vc.expr, expr.LogicExpr) and vc.expr.op == '->':
                        ...
                        if isinstance(expr1, expr.LogicExpr) and expr1.op == '&':
                            ...
                            for r_expr in right_exprs:
                                info.vcs.append(Predicate(expr.imp(expr0, r_expr), vc.pos, vc.path))
                            info.vcs.remove(vc)
