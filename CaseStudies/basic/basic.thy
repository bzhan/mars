...
lemma b7:
  "\<Turnstile>
    {\<lambda>s tr. s X \<ge> 0 \<and> s Y \<ge> 1 \<and> s T = 0}
      X ::= (\<lambda>s. s X + 1);
...
lemma b8:
  "\<Turnstile>
    {\<lambda>s tr. s X \<ge> 0 \<and> s Y \<ge> 0 \<and> s T = 0}
...
lemma b9:
  "\<Turnstile>
    {\<lambda>s tr. s X > 0 \<and> s Y > 0 \<and> s T = 0}
...
lemma b12:
  ...
  apply(rule Valid_strengthen_post)
   ...
   apply(rule Valid_weaken_pre)
    prefer 2
    apply (rule DC'[where P="\<lambda>tr. True" and init="\<lambda>s. 0 \<le> s X \<and> 0 \<le> s Y" and c="\<lambda>s. s Y \<ge> 0" and
                          d="\<lambda>s . s X \<ge> 0"])
     apply (rule Valid_weaken_pre)
      prefer 2
...
lemma b13:
  "\<Turnstile>
    {\<lambda>s tr. s X \<ge> 0 \<and> s Y \<ge> 0 \<and> s Z \<ge> 0}
  ...
  apply(rule Valid_strengthen_post)
   ...
   apply(rule Valid_weaken_pre)
    prefer 2
    apply(rule DC'[where P = "\<lambda>tr. True" and init = "\<lambda>s . 0 \<le> s X \<and> 0 \<le> s Y \<and> s Z \<ge> 0" and
                         c = "\<lambda>s. s Y \<ge>0" and d = "\<lambda>s . s X \<ge>0"])
     apply(auto simp add: entails_def)
   apply(rule Valid_strengthen_post)
    prefer 2
    apply(rule Valid_weaken_pre)
     prefer 2
     apply(rule DC'[where P = "\<lambda>tr. True" and init = "\<lambda>s . 0 \<le> s X \<and> 0 \<le> s Y \<and> s Z \<ge> 0" and
                          c = "\<lambda>s. s Z \<ge>0" and d = "\<lambda>s . s Y \<ge>0"])
      apply(auto simp add: entails_def)
    apply(rule Valid_weaken_pre)
     prefer 2
     apply(rule Valid_inv_s_tr_ge)
...
lemma b14:
  "\<Turnstile>
    {\<lambda>s tr. s X \<ge> 0 \<and> s Y \<ge> 0 \<and> s Z \<ge> 0 \<and> s J \<ge> 0}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z, Z := \<lambda>s. s J,
                           J := \<lambda>s. (s J)^2, T := \<lambda>s. 1)))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s X \<ge> 0}"
  apply(rule Valid_pre_cases[where Q = "(\<lambda>s. s T < P)"])
   prefer 2
  ...
  apply(rule Valid_strengthen_post)
   ...
   apply(rule Valid_weaken_pre)
    prefer 2
    apply(rule DC'[where P="\<lambda>tr. True" and init="\<lambda>s . 0 \<le> s X \<and> 0 \<le> s Y \<and> s Z \<ge> 0 \<and> s J \<ge> 0" and
                         c="\<lambda>s. s Y \<ge>0" and d="\<lambda>s . s X \<ge>0"])
     apply(auto simp add: entails_def)
   apply(rule Valid_strengthen_post)
    prefer 2
    ...
    apply(rule Valid_strengthen_post)
     ...
     apply(rule Valid_weaken_pre)
      prefer 2
      apply(rule DC'[where P="\<lambda>tr. True" and init="\<lambda>s. 0 \<le> s X \<and> 0 \<le> s Y \<and> s Z \<ge> 0 \<and> s J \<ge> 0" and
                           c="\<lambda>s. s J \<ge>0" and d="\<lambda>s. s Z \<ge>0"])
       apply(auto simp add: entails_def)
     apply(rule Valid_weaken_pre)
      prefer 2
      apply(rule Valid_inv_s_tr_ge)
...
lemma b19:
  ...
  apply(rule Valid_strengthen_post)
   ...
   apply(rule Valid_weaken_pre)
    prefer 2
    apply(rule DC'[where P="\<lambda>tr. True" and init="\<lambda>s . 0 \<le> s X \<and> 0 \<le> s Y " and
                         c="\<lambda>s. s Y \<ge>0" and d="\<lambda>s . s X \<ge>0"])
     apply(auto simp add: entails_def)
   apply(rule Valid_weaken_pre)
    prefer 2
...
lemma b24:
  "\<Turnstile>
    {\<lambda>s tr. s Z ^ 2 + s J ^ 2 - s L ^ 2 * s M ^ 2 = 0 \<and> s Z + s L * s Y = 0 \<and> s J - s L * s X = 0}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Z, Y := \<lambda> s. s J, Z := \<lambda> s. - s L * s J,
                           J := \<lambda> s. s L * s Z , T := \<lambda>s. 1)))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s Z ^ 2 + s J ^ 2 - s L ^ 2 * s M ^ 2 = 0 \<and> s Z + s L * s Y = 0 \<and> s J - s L * s X = 0}"
  apply(rule Valid_post_and)
   apply(rule Valid_weaken_pre)
...
lemma b25:
  "\<Turnstile>
    {\<lambda>s tr. s Z \<ge> 0 \<and>  s X = 0  \<and>  s Y = 3}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda> s. - s X * s Z ^ 2 - 2 * s Z * s Y,
                           T := \<lambda>s. 1)))
           (\<lambda>s. s T < P)
  ...
  apply(rule Valid_strengthen_post)
   ...
   apply(rule Valid_weaken_pre)
    prefer 2
    apply(rule DC'[where P = "\<lambda>tr. True" and
                         init = "\<lambda>s . s Z \<ge> 0 \<and>  s X = 0  \<and>  s Y = 3 " and
                         c = "\<lambda>s. s Z \<ge>0" and
                         d = "\<lambda> s. s Z ^ 2 * s X ^ 2 + s Y ^ 2 \<le> 9"])
     apply(rule Valid_weaken_pre)
      prefer 2
      apply(rule Valid_inv_s_tr_ge)
       apply clarify
...
lemma b26:
  "\<Turnstile>
    {\<lambda>s tr. s X ^ 3 > 5 \<and> s Y > 2}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s X ^ 3 + s X ^ 4, Y := \<lambda> s. 5 * s Y + s Y ^ 2,
                           T := \<lambda>s. 1)))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s X ^ 3 > 5  \<and>  s Y > 2 }"    
  apply(rule Valid_post_and)
...
lemma b27:
  "\<Turnstile>
    {\<lambda>s tr. s X \<ge> 1  \<and> s Y = 10 \<and> s Z = -2}
  ...
  "\<Turnstile>
    {\<lambda>s tr. s X ^ 4 * s Y ^ 2 + s X ^ 2 * s Y ^ 4 - 3 * s X ^ 2 * s Y ^ 2 + 1 \<le> c}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. 2 * s X ^ 4 * s Y + 4 * s X ^ 2 * s Y ^ 3 - 6 * s X ^ 2 * s Y, 
                           Y := \<lambda> s. - 4 * s X ^ 3 * s Y ^ 2 - 2 * s X * s Y ^ 4  + 6 * s X * s Y ^ 2,
                           T := \<lambda>s. 1)))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s X ^ 4 * s Y ^ 2 + s X ^ 2 * s Y ^ 4 - 3 * s X ^ 2 * s Y ^ 2 + 1 \<le> c}"
...
lemma b29:
  "\<Turnstile>
    {\<lambda>s tr. s X + s Z = 0}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. A * s X ^ 2 + B * s X, Z := \<lambda> s. A * s Z * s X + B * s Z,
                           T := \<lambda> s. 1)))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s X + s Z = 0 }"
...
lemma b33:
  assumes "a^2 \<le> 4"
    and "b^2 \<ge> 1/3"
  shows "\<Turnstile>
    {\<lambda>s tr. s L \<ge> 0 \<and> s M \<ge> 0 \<and> s L ^ 2 * s X ^ 2 + s Y ^ 2 - s N \<le> 0}
     Rep (
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda> s . s Y , Y := \<lambda>s. - (s L ^ 2) * s X - 2 * s M * s L * s Y))) Con;
      IF (\<lambda> s . s X = s Y * a) THEN
        (L ::= (\<lambda> s. 2 * s L);
         M ::= (\<lambda> s. 1/2 * s M);
         N ::= (\<lambda> s. s N * (4 * s L ^ 2 + 1)/(s L ^ 2 + 1)))
      ELSE
        IF (\<lambda> s . s X = s Y * b) THEN
          (L ::= (\<lambda> s. 1/2 * s L);
           M ::= (\<lambda> s. 2 * s M);
           N ::= (\<lambda> s. s N * (s L ^ 2 + 1)/(2 * s L ^ 2 + 1)))
        ELSE 
          Skip
        FI
      FI
  ...
  subgoal
    ...
    subgoal
      apply(rule Valid_weaken_pre)
       prefer 2
       apply(rule DC'''[where init="\<lambda>s. 0 \<le> s L \<and> 0 \<le> s M \<and> (s L)\<^sup>2 * (s X)\<^sup>2 + (s Y)\<^sup>2 - s N \<le> 0" and
                              P = "\<lambda> tr . True"])
      subgoal 
        apply(rule Valid_weaken_pre)
         prefer 2
  ...
  subgoal
    ...
    subgoal
      ...
      subgoal
        ...
        using assms
        using mult_right_mono 
          apply (metis divide_eq_eq_numeral1(1) divide_le_eq_numeral1(1) mult_cancel_right1
                       nonzero_mult_div_cancel_left zero_neq_numeral)
        apply simp
        by linarith
      apply(rule Valid_strengthen_post)
      prefer 2
...
lemma b36:
  "\<Turnstile>
    {\<lambda>s tr. 1/2 * s X ^ 2 - 1/2 * s Y ^ 2 >=  a}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda> s . s Y + s X * s Y ^ 2, Y := \<lambda> s. - s X + s X ^ 2 * s Y)))
  ...
  subgoal
    apply(rule Valid_weaken_pre)
     prefer 2
     apply(rule DC''[where P="\<lambda>tr. True" and c="(\<lambda>s. s X \<ge> 0 \<and> s Y \<ge> 0)" and
                           init="\<lambda> s. 1/2 * s X ^ 2 - 1/2 * s Y ^ 2 \<ge> a"])
    subgoal
      apply(rule Valid_weaken_pre)
       prefer 2
       apply(rule Valid_strengthen_post)
...
lemma b37:
  "\<Turnstile>
    {\<lambda>s tr. - s X * s Y \<ge> a}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda> s . s X - s Y + s X * s Y , Y := \<lambda> s. - s Y - s Y ^ 2,
                           T := \<lambda> s. 1 )))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. - s X * s Y \<ge> a }"
  apply(rule Valid_inv_s_ge)
...
lemma b39:
  "\<Turnstile>
    {\<lambda>s tr. s X ^ 3 \<ge> -1 \<and> s Y ^ 5 \<ge> 0 }
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda> s . (s X - 3) ^ 4  + s Y ^ 5 , Y := \<lambda>  s. s Y ^ 2,
                           T := \<lambda> s. 1 )))
           (\<lambda>s. s T < P)
    {\<lambda>s tr. s X ^ 3 \<ge> -1 \<and> s Y ^ 5 \<ge> 0}"
  apply(rule Valid_pre_cases[where Q = "(\<lambda>s. s T < P)"])
  ...
  subgoal
    apply(rule Valid_weaken_pre)    
     prefer 2
     apply(rule DC''[where P="\<lambda>tr. True" and init="\<lambda>s. s X ^ 3 \<ge> -1 \<and> s Y ^ 5 \<ge> 0" and
                           c="\<lambda>s. s Y ^ 5 \<ge> 0"])
    subgoal
      apply(rule Valid_weaken_pre)
       prefer 2
       apply(rule Valid_inv_tr_ge)
...
lemma b41:
  ...
  shows "\<Turnstile>
    {\<lambda>s tr. s Y \<ge> 0 }
      Rep (
        IChoice (IChoice (Z ::= (\<lambda>s. A)) (Z ::= (\<lambda>s. 0))) (Z ::= (\<lambda>s. - B));
...
lemma b42:
  assumes "A > 0" and "B > 0"
  shows "\<Turnstile>
    {\<lambda>s tr. 0 \<le> s Y \<and> s X + 1 / (2 * B) * (s Y)\<^sup>2 < S}
      Rep (
        IF (\<lambda>s. s X + 1 / (2 * B) * (s Y)\<^sup>2 < S) THEN
          Z ::= (\<lambda>s. A)
        ELSE
          IF (\<lambda>s. s Y = 0) THEN
            Z ::= (\<lambda>s. 0)
          ELSE
            Z ::= (\<lambda>s. - B)
          FI
        FI;
        IChoice (Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z))) (\<lambda>s. s X + 1 / (2 * B) * (s Y)\<^sup>2 < S))
                (Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z))) (\<lambda>s. S < s X + 1 / (2 * B) * (s Y)\<^sup>2))
      )
...
lemma b43:
  ...
  shows "\<Turnstile>
    {\<lambda>s tr. s Y \<le> V}
      Rep (
        Cond (\<lambda>s. s Y = V)(Z ::= (\<lambda> s . 0)) (Z ::= (\<lambda> s . A));
...
lemma b45:
  ...
  shows "\<Turnstile>
    {\<lambda>s tr. s Y \<le> V}
      Rep (
       IF (\<lambda>s. s Y = V) THEN
         Z ::= (\<lambda>s. 0)
       ELSE
         Z ::= (\<lambda>s. A)
       FI;
       IChoice (Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z))) (\<lambda>s. s Y < V))
               (Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z))) (\<lambda>s. V < s Y)))
    {\<lambda>s tr. s Y \<le> V}"
  apply(rule Valid_rep)
  apply(rule Valid_seq)
   apply(rule Valid_cond_sp)
...
lemma b46:
  ...
  shows "\<Turnstile>
   {\<lambda>s tr. 0 \<le> s Y \<and> s X + (s Y)\<^sup>2 / (2 * B) \<le> S}
     Rep (
      IF (\<lambda>s. s X + (s Y)\<^sup>2 / (2 * B) + (A / B + 1) * (A / 2 * ep\<^sup>2 + ep * s Y) \<le> S) THEN
        Z ::= (\<lambda>s. A)
      ELSE
        Z ::= (\<lambda>s. - B)
      FI;
      T ::= (\<lambda>s. 0);
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z, T := \<lambda>s. 1)))
           (\<lambda>s. 0 < s Y \<and> s T < ep)
     )
   {\<lambda>s tr. s X \<le> S}"
  apply(rule Valid_strengthen_post)
   prefer 2
  ...
  subgoal
    ...
    apply(rule Valid_strengthen_post)
     prefer 2
     apply(rule Valid_ode_unique_solution_s_sp[where d="\<lambda>s. ep" and
          p ="\<lambda>s t. s(X := s X + s Y * t + 1/2 * s Z * t^2, Y := s Y + s Z * t, T := s T + t) "])
    ...
    subgoal
    proof-
      have b1:"bounded_linear (\<lambda>(v::vec). (\<chi> a. if a = T then 0 else
                                                if a = X then v $ Y else
                                                if a = Y then v $ Z else 0))"
      ...
      show ?thesis 
        ...
        apply(rule c1_implies_local_lipschitz[where
              f'="(\<lambda>_. Blinfun (\<lambda>v. (\<chi> a. if a = T then 0 else
                                          if a = X then v $ Y else if a = Y then v $ Z else 0)))"])
  ...
  subgoal
    ...
    apply(rule Valid_strengthen_post)
     ...
     apply(rule Valid_ode_unique_solution_s_sp[where
          d="\<lambda>s. if s Y \<le> ep * B then s Y/B else ep" and
          p ="\<lambda> s t . s(X := s X + s Y * t + 1/2 * s Z * t^2, Y := s Y + s Z * t, T := s T + t)"])
    ...
    proof-
      ...
      show ?thesis 
        unfolding state2vec_def vec2state_def fun_upd_def
        apply(rule c1_implies_local_lipschitz[where
              f'="(\<lambda>_. Blinfun (\<lambda>v. (\<chi> a. if a = T then 0 else if a = X then v $ Y else
                                          if a = Y then v $ Z else 0)))"])
        apply (auto simp add: bounded_linear_Blinfun_apply[OF b1])
        subgoal premises pre for t x
          unfolding has_derivative_def apply(auto simp add: b1)
...
lemma b49:
  ...
  shows "\<Turnstile>
    {\<lambda>s t. s Y \<ge> 0 \<and> (5/4 * (s X - r)^ 2 + (s X - r) * s Y / 2 + (s Y ^ 2)/4 < c)}
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y , Y := \<lambda>s. - p * (s X - r) - d * s Y, T := \<lambda> s . 1)))
           (\<lambda>s. s T < P)
  ...
  apply(auto simp add:state2vec_def entails_def)
  using assms apply auto
  apply (auto simp add: algebra_simps power_add power2_eq_square power2_diff add_divide_distrib
                        diff_divide_distrib)
...
lemma b50:
  assumes "p = 2"
    and "d = 3"
  shows "\<Turnstile>
    {\<lambda>s t. s Y \<ge> 0 \<and> s X - s M \<ge> 0 \<and> s X \<le> S \<and> (s M + S) - 2 * s L = 0 \<and>
           (5/4 * (s X - s L)^ 2 + (s X - s L) * s Y / 2 + (s Y ^ 2)/4 - ((S - s M)/2)^2 < 0)}
      Rep (
        Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s . s Y , Y := \<lambda>s. - p * (s X - s L) - d * s Y ))) (\<lambda>s. s Y > 0)
      )
    {\<lambda> s t.  s X \<le> S}"
  apply (rule Valid_weaken_pre[where
        P'="\<lambda>s t. s Y \<ge> 0 \<and> s X - s M \<ge> 0 \<and> (s M + S) - 2 * s L = 0 \<and>
                     (5/4 * (s X - s L) ^ 2 + (s X - s L) * s Y / 2 + (s Y ^ 2)/4 - ((S - s M)/2)^2 < 0)"])
  ...
  subgoal
    ...
    subgoal
      apply(rule Valid_weaken_pre)
       prefer 2
       apply(rule DC'''[where P= "\<lambda>tr. True" and
                init="\<lambda>s. 0 \<le> s Y \<and> s X - s M \<ge> 0 \<and>
                      (s M + S)- 2 * s L = 0 \<and>
                      (5/4 * (s X - s L)^ 2 + (s X - s L) * s Y / 2 + (s Y ^ 2)/4 - ((S - s M)/2)^2 < 0)"])
      subgoal 
        apply (rule Valid_weaken_pre)
      ...
      subgoal
        apply(rule Valid_weaken_pre)
         prefer 2
         apply (rule DC'''[where P="\<lambda>tr. True" and
                   init = "\<lambda>s. 0 \<le> s Y \<and> s X - s M \<ge> 0 \<and> s X \<le> S \<and>
                               (s M + S)- 2 * s L = 0 \<and>
                               (5/4 * (s X - s L)^ 2 + (s X - s L) * s Y / 2 + (s Y ^ 2)/4 - ((S - s M)/2)^2 < 0)"])
        subgoal
          apply (rule Valid_weaken_pre)
        ...
        subgoal
          apply(rule Valid_weaken_pre)
           prefer 2
           apply (rule DC'''[where P="\<lambda>tr. True" and
                    init="\<lambda>s. 0 \<le> s Y \<and> s X - s M \<ge> 0 \<and> s X \<le> S \<and> (s M + S)- 2 * s L = 0 \<and>
                              (5/4 * (s X - s L)^ 2 + (s X - s L) * s Y / 2 + (s Y ^ 2)/4 - ((S - s M)/2)^2 < 0)"])
          subgoal
            apply (rule Valid_weaken_pre)
             prefer 2
...
lemma b52:
  "\<Turnstile>
    {\<lambda>s t. s Y \<ge> 0 \<and> s Z \<ge> 0 }
      Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda> s . s Y , Y := \<lambda>  s. s Z ))) (\<lambda>s. s Y > 0)
    {\<lambda>s t. s Y \<ge>0}"
  apply(rule Valid_pre_cases[where Q = "\<lambda> s. s Y > 0"])
   apply(rule Valid_weaken_pre)
...
lemma b55:
  ...
  shows "\<Turnstile>
    {\<lambda>s tr. 0 \<le> s Y \<and> (s Y)\<^sup>2 \<le> (S - s X) * (2 * B)}
      Rep (
        IF (\<lambda>s. (A + B) * (A * ep\<^sup>2 + 2 * ep * s Y) + (s Y)\<^sup>2 \<le> (S - s X) * 2 * B) THEN
          Z ::= (\<lambda>s. A)
        ELSE
          Z ::= (\<lambda>s. - B)
        FI;
        T ::= (\<lambda>s. 0);
        Cont (ODE ((\<lambda>_ _. 0)(X := \<lambda>s. s Y, Y := \<lambda>s. s Z, T := \<lambda>s. 1))) (\<lambda>s. 0 < s Y \<and> s T < ep)
      )
    {\<lambda>s tr. s X \<le> S}"
  apply(rule Valid_strengthen_post)
   prefer 2
