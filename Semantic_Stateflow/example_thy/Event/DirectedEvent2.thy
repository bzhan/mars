...
definition Chart_AM_B_A2 :: state where " Chart_AM_B_A2 = State [''AM'', ''B'', ''A2'']
  (print1 ''en_B2_A2'' )
  (SKIP)
  (print1 ''ex_B2_A2'' )
  []
  [Trans (P [''AM'', ''B'', ''A2'']) (S [''E_two'']) (Bc True) (send2 ''E_two'' False [''AM'', ''A''] ) (SKIP) (P [''AM'', ''B'', ''A1''])]
  (No_Composition)"

definition Chart_AM_B_A1 :: state where " Chart_AM_B_A1 = State [''AM'', ''B'', ''A1'']
  (print1 ''en_B1_A1'' )
  (SKIP)
  (print1 ''ex_B1_A1'' )
  []
  [Trans (P [''AM'', ''B'', ''A1'']) (S [''E_one'']) (Bc True) (send2 ''E_one'' False [''AM'', ''C''] ) (SKIP) (P [''AM'', ''B'', ''A2''])]
  (No_Composition)"

definition f_Chart_AM_B :: string2state where 
" f_Chart_AM_B = 
(λstr. if str = ''A2'' then Chart_AM_B_A2 else 
if str = ''A1'' then Chart_AM_B_A1 else 
No_State )"
...
definition Chart_AM_B :: state where " Chart_AM_B = State [''AM'', ''B'']
  ...
  []
  []
  (Or ([Trans (NONE) (S []) (Bc True) (SKIP) (SKIP) (P [''AM'', ''B'', ''A1''])])
 (False) (f_Chart_AM_B))"

definition Chart_AM_C_A2 :: state where " Chart_AM_C_A2 = State [''AM'', ''C'', ''A2'']
  (print1 ''en_C2_A2'' )
  (SKIP)
  (print1 ''ex_C2_A2'' )
  []
  [Trans (P [''AM'', ''C'', ''A2'']) (S []) ((V ''data'') [==] (N 1)) (send2 ''E_two'' False [''AM'', ''B''] ) (SKIP) (P [''AM'', ''C'', ''A1''])]
  (No_Composition)"

definition Chart_AM_C_A1 :: state where " Chart_AM_C_A1 = State [''AM'', ''C'', ''A1'']
  (print1 ''en_C1_A1'' )
  (SKIP)
  (print1 ''ex_C1_A1'' )
  []
  [Trans (P [''AM'', ''C'', ''A1'']) (S [''E_one'']) (Bc True) (SKIP) (SKIP) (P [''AM'', ''C'', ''A2''])]
  (No_Composition)"

definition f_Chart_AM_C :: string2state where 
" f_Chart_AM_C = 
(λstr. if str = ''A2'' then Chart_AM_C_A2 else 
if str = ''A1'' then Chart_AM_C_A1 else 
No_State )"
...
definition Chart_AM_C :: state where " Chart_AM_C = State [''AM'', ''C'']
  ...
  []
  []
  (Or ([Trans (NONE) (S []) (Bc True) (SKIP) (SKIP) (P [''AM'', ''C'', ''A1''])])
 (False) (f_Chart_AM_C))"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''en_A1'', ''en_B1_A1'', ''en_C1_A1'', ''ex_C1_A1'', ''en_C2_A2'', ''ex_B1_A1'', ''en_B2_A2'', ''ex_A1'', ''en_A2'', ''ex_A2'', ''en_A1'', ''ex_B2_A2'', ''en_B1_A1'', ''ex_C2_A2'', ''en_C1_A1''], ?o2)) (?I))"
  unfolding Chart_AM_A_A1_def Chart_AM_A_A2_def f_Chart_AM_A_def Chart_AM_A_def Chart_AM_B_A2_def 
Chart_AM_B_A1_def f_Chart_AM_B_def Chart_AM_B_def Chart_AM_C_A2_def Chart_AM_C_A1_def 
