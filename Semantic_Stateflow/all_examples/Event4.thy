...
definition Chart_A_A1 :: state where " Chart_A_A1 = State [''A'', ''A1'']
  ...
  (SKIP)
  []
  [Trans (P [''A'', ''A1'']) (S [''E'']) (Bc True) ((print1 ''a1'' ); (send1 ''F'' False)) (SKIP) (J [''A'', ''40''])]
  (No_Composition)"
...
definition Chart_C_C1 :: state where " Chart_C_C1 = State [''C'', ''C1'']
  ...
  (SKIP)
  []
  [Trans (P [''C'', ''C1'']) (S [''F'']) (Bc True) (print1 ''c'' ) (SKIP) (P [''C'', ''C2''])]
  (No_Composition)"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''b'', ''a1'', ''c'', ''en_C2'', ''a2'', ''en_A2'', ''tb'', ''en_B2''], ?o2)) (?I))"
