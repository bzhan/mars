...
definition Chart_A_A1 :: state where " Chart_A_A1 = State [''A'', ''A1'']
  ...
  (SKIP)
  []
  [Trans (P [''A'', ''A1'']) (S [''E'']) (Bc True) (SKIP) (SKIP) (P [''A'', ''A3'']), Trans (P [''A'', ''A1'']) (S []) (Bc True) (print1 ''ca'' ) ((send1 ''E'' True); (print1 ''ta'' )) (P [''A'', ''A2''])]
  (No_Composition)"
...
definition Root :: comp where " Root = And [''A'']
 (f_Chart)"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''ca'', ''ta'', ''en_A2''], ?o2)) (?I))"
