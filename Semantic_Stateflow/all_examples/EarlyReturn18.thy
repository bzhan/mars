...
definition Chart_S :: state where " Chart_S = State [''S'']
  ...
  (print1 ''duS'' )
  (print1 ''exS'' )
  [Trans (P [''S'']) (S []) (Bc True) (print1 ''ca1'' ) (SKIP) (J [''S'', ''15''])]
...
" g = (λ str. if str = [''S'', ''15''] then [Trans (J [''S'', ''15'']) (S []) (Bc True) ((send1 ''E'' False); (print1 ''ca2'' )) (SKIP) (P [''S''])] else 
[])"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''enS'', ''duS'', ''ca1'', ''exS'', ''enT''], ?o2)) (?I))"
