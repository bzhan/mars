"""Simulation for Hpical programs.
...
from Hpicalculus.expr import AExpr, AVar, AConst, OpExpr, FunExpr, IfExpr, \
...
# from Hpicalculus import pprint
# from Hpicalculus import graph_plot
import numpy as np
...
class SimulatorAssertionException(Exception):
    """Failure of a test in the Hpical program."""
...
def eval_expr(expr, state):
    ...
    if expr is None:
        ...
        if len(expr.exprs) == 1:
            ...
            e1, e2 = eval_expr(expr.exprs[0], state), eval_expr(
                expr.exprs[1], state)
            if expr.op == '+':
                return e1 + e2
            elif expr.op == '-':
                ...
                if isinstance(e2, Decimal):
                    if e2 != int(e2):
                        raise SimulatorException(
                            "When evaluating %s: %s is not an integer" % (expr, e2))
        ...
        if expr.fun_name == "min":
            ...
            if len(a) == 0:
                raise SimulatorException(
                    'When evaluating %s: argument is empty' % expr)
            return a[:-1]
            ...
            if args[0] < 0:
                raise SimulatorException(
                    'When evaluating %s: argument %s less than zero' % (expr, args[0]))
            return math.sqrt(args[0])
        elif expr.fun_name == "bernoulli":
            assert len(args) == 1
            if not (args[0] >= 0 and args[0] <= 1):
                raise SimulatorException(
                    'When evaluating %s: argument %s not in range' % (expr, args[0]))
            ...
            if args[0] > args[1]:
                raise SimulatorException(
                    'When evaluating %s: %s > %s' % (expr, args[0], args[1]))
            return random.uniform(args[0], args[1])
        elif expr.fun_name == "unidrnd":
            assert len(args) == 1
            if args[0] <= 0:
                raise SimulatorException(
                    'When evaluating %s: %s <= 0' % (expr, args[0]))
            ...
            raise SimulatorException(
                "When evaluating %s: unrecognized function" % expr)
        ...
        if not i < len(a):
            raise SimulatorException(
                'When evaluating %s: array out of bounds error' % expr)
        return a[i]
        ...
        if not isinstance(a, dict) or expr.field not in a:
            raise SimulatorException(
                'When evaluating %s: field not found' % expr)
        return a[expr.field]
...
def eval_channel(ch_name, state):
    ...
    return Hpical.Channel(ch_name.name, tuple(args))
...
def get_ode_delay(hp, state):
    ...
    def test_cond(e):
        ...
        def event(t, y): return event_gen(t, y, e)
        ...
        cur_delay = 100
        for delay in delays:
            sol = solve_ivp(ode_fun, [0, delay], y0, events=[
                            event], rtol=1e-5, atol=1e-7)
            if sol.t[-1] < delay:
...
class SessionFrame:
    def __init__(self, ses_chs,inner_chs):
        self.ses_chs=ses_chs # q in s[q], in s[p][q] for s
        self.inner_chs =inner_chs #s[p] in s[p][q] for a

class SessionMap:
    def __init__(self):
        # dict['session_name']=SessionFrame
        self.sessions = dict()
    ...
    def renewinnerpos(self, hp, procs):
        if self.callstack[-1].pos is None:
            ...
            for i in range(len(pos)-length, len(pos)+1):
                sub_hp = get_pos(
                    hp, pos[:i], self.callstack[-1].rec_vars, procs)
                if sub_hp.type == "recursion":
                    rec_list.append(i)
            if len(rec_list) >= 2:
                self.callstack[-1].innerpos = pos[:rec_list[0]
                                                  ] + pos[rec_list[-1]:]
                self.callstack[-1].innerpos = self.callstack[-1].innerpos[(
                    len(pos)-length):]
    ...
    def getinfo(self, hp, procs):
        ...
        while index >= 0:
            ...
            if innerpos is None:
                ...
                callstack_info['innerpos'].append(
                    'p' + ','.join(str(p) for p in innerpos))
            else:
                callstack_info['innerpos'].append(
                    'p' + ','.join(str(p) for p in innerpos[:-1]))
            if self.callstack[index].proc_name is None:
                callstack_info['procedure'].append(
                    self.callstack[index].proc_name)
            else:
                callstack_info['procedure'].append(
                    self.callstack[index].proc_name)
            index = index - 1
...
def get_pos(hp, pos, rec_vars=None, procs=None):
    ...
    rec_vars: mapping from recursion variables to Hpical processes.
    procs: mapping from procedure names to Hpical processes.
    ...
    if hp.type == 'sequence':
        ...
        if hp.name in rec_vars:
            ...
            raise SimulatorException(
                "Unrecognized process variable: " + hp.name)
...
class SimInfo:
    """Represents a (non-parallel) Hpical program together with
    ...
    def __init__(self, name, hp, *, outputs=None, procedures=None, pos="start", state=None):
        ...
        #     assert isinstance(k, str) and isinstance(v, Hpical.Procedure)
    ...
    def exec_assign(self, lname, val, hp):
        ...
        if isinstance(lname, AVar):
            ...
            if not isinstance(v, list):
                raise SimulatorException(
                    '%s is not a list, when executing %s' % (v, hp))
            if idx >= len(v):
                raise SimulatorException(
                    'Array index %s out of bounds, when executing %s' % (idx, hp))
            v[idx] = copy.deepcopy(val)
        elif isinstance(lname, FieldNameExpr):
            v = eval_expr(lname.expr, self.state)
            if lname.field not in v:
                raise SimulatorException(
                    'Field %s does not exist, when executing %s' % (lname.field, hp))
    ...
    def exec_step(self, infos):
        ...
        rec_vars = dict()
        cur_hp = get_pos(self.hp, self.callstack.top_pos(),
                         rec_vars, procedures_list)
        if cur_hp.type == "skip":
            self.callstack = step_pos(
                self.hp, self.callstack, self.state, rec_vars, procedures_list)
            self.reason = None

        elif cur_hp.type == "assign":
            # Perform assignment
            if isinstance(cur_hp.var_name, (AExpr, function.DirectName)):
                self.exec_assign(cur_hp.var_name, eval_expr(
                    cur_hp.expr, self.state), cur_hp)
            else:
                # Multiple assignment
            ...
            for i in range(len(self.callstack.top_pos())):
                hp = get_pos(self.hp, self.callstack.top_pos()
                             [:i], rec_vars, procedures_list)
            ...
            raise SimulatorException(
                "Unrecognized process variable: " + cur_hp.name)

        elif cur_hp.type == "input_channel":
            # Waiting for input
            self.reason = {
                "comm": [(eval_channel(cur_hp.ch_name, self.state), "?")]}
            ...
            delay = eval_expr(cur_hp.delay, self.state) - \
                self.callstack.top_pos()[-1]
            if delay < 0:
                raise SimulatorException(
                    "When executing %s: delay %s less than zero" % (cur_hp, delay))
            ...
            for io_comm, rest in cur_hp.io_comms:
                if io_comm.type == "input_channel":
                    comms.append(
                        (eval_channel(io_comm.ch_name, self.state), "?"))
                else:
                    comms.append(
                        (eval_channel(io_comm.ch_name, self.state), "!"))
            ...
            for comm_hp, out_hp in cur_hp.io_comms:
                if comm_hp.type == "input_channel":
                    comms.append(
                        (eval_channel(comm_hp.ch_name, self.state), "?"))
            ...
            pos = self.callstack.top_pos() + (len(cur_hp.if_hps),) + \
                start_pos(cur_hp.else_hp)
            self.callstack.renew(pos, rec_vars)
            self.reason = None
    ...
    def exec_input_comm(self, ch_name, x, inst=None):
        ...
        if cur_hp.type == "input_channel":
            ...
            self.callstack = step_pos(
                self.hp, self.callstack, self.state, rec_vars, self.procedures)
            ...
            for i, (comm_hp, out_hp) in enumerate(cur_hp.io_comms):
                if comm_hp.type == "input_channel" and eval_channel(comm_hp.ch_name, self.state) == ch_name:
                    if comm_hp.var_name is None:
                        if x is not None:
                            raise SimulatorException(
                                comm_hp, "input value is not None")
                    else:
                        if x is None:
                            raise SimulatorException(
                                comm_hp, "input value is None")
                        self.exec_assign(comm_hp.var_name, x, comm_hp)
                    pos = self.callstack.top_pos() + (i,) + start_pos(out_hp)
                    self.callstack.renew(pos, rec_vars)
                    return
    ...
    def exec_delay(self, delay, *, time_series=None):
        ...
        if cur_hp.type in ["input_channel", "output_channel", "select_comm"]:
            ...
            if self.reason['delay'] == delay:
                ...
                pos = self.callstack.top_pos(
                )[:-1] + (self.callstack.top_pos()[-1] + delay,)
                self.callstack.renew(pos, rec_vars)
            ...
            if delay != 0.0:
                ...
                if time_series is not None:
                    for i in range(len(sol.t)):
                        ...
                        time_series.append(
                            {'time': t_eval[i], 'state': copy.deepcopy(self.state)})
    ...
    for out_ch in out_ready:
        for in_ch in in_ready:
            ...
            if match_res is not None:
                ...
                assert in_ch.subst(in_inst) == out_ch.subst(
                    out_inst)  # instantiated channel
...
def exec_parallel(infos, *, num_io_events=None, num_steps=3000, num_show=None,
    ...
    def log_event(ori_pos, **xargs):
        ...
        if 'str' in new_event:
            new_event['str'] = new_event['str'].encode(
                'utf-8').decode('unicode_escape').strip()
        ...
        for info in infos:
            ...
            cur_info[info.name] = {
                'callstack': info_callstack, 'statenum': state_num}
    ...
    if start_event:
        ...
        start_pos = dict((info.name, disp_of_callstack(info))
                         for info in infos)
    ...
    for _ in range(num_io_events):
        ...
        for info in infos:
            while info.callstack.top_pos() is not None and not num_event >= start_id + num_steps:
                ...
                if info.reason is None:
                    ...
                    log_event(ori_pos=ori_pos, type="log",
                              str='log ' + info.reason['log'])
                elif 'warning' in info.reason:
                    log_event(ori_pos=ori_pos, type="warning",
                              str="warning: " + info.reason['warning'])
                elif 'error' in info.reason:
                    log_event(ori_pos=ori_pos, type="error",
                              str="error: " + info.reason['error'])
                    break
                else:
                    break
        ...
        if event == "deadlock":
            ...
            ori_pos = dict((infos[p].name, disp_of_callstack(infos[p]))
                           for p in delay_pos)
            ...
            for info in infos:
                ...
                for entry in series:
                    log_time_series(
                        info, res['time'] + entry['time'], entry['state'])
                log_time_series(info, res['time'] + min_delay, info.state)

            log_event(ori_pos=ori_pos, type="delay",
                      delay_time=min_delay, str=trace_str)
            res['time'] += min_delay
        else:  # event[0] == "comm"
...
def check_comms(infos):
    """Given a list of Hpical infos, check for potential mismatch of
    ...
    for info in infos:
        ...
        for ch_name, direction in Hpical.get_comm_chs(info.hp):
    ...
    for ch_name, hp_names in comm_in_map.items():
        if len(hp_names) >= 2:
            warnings.append("Warning: input %s used in more than one process: %s" % (
                ch_name, ', '.join(hp_names)))
        if ch_name not in comm_out_map:
            warnings.append(
                "Warning: input channel %s has no corresponding output" % ch_name)
        elif hp_names[0] in comm_out_map[ch_name]:
            warnings.append(
                "Warning: input and output channel %s in the same process" % ch_name)

    for ch_name, hp_names in comm_out_map.items():
        if len(hp_names) >= 2:
            warnings.append("Warning: output %s used in more than one process: %s" % (
                ch_name, ', '.join(hp_names)))
        if ch_name not in comm_in_map:
            warnings.append(
                "Warning: output channel %s has no corresponding input" % ch_name)
