...
class IfExpr(AExpr):
    def __init__(self, cond, expr1, expr2, meta=None):
        ...
        assert isinstance(cond, BExpr) and isinstance(
            expr1, AExpr) and isinstance(expr2, AExpr)
        self.cond = cond
        self.expr1 = expr1
        self.expr2 = expr2
        self.meta = meta
...
class DictExpr(AExpr):
    ...
    def __init__(self, *args, meta=None):
        ...
        self.dict = dict()
        args = tuple(args)
        assert all(isinstance(k, str) and isinstance(v, AExpr)
                   for k, v in args)
        for k, v in args:
            self.dict[k] = v
        self.meta = meta
...
def conj(*args):
    ...
    assert isinstance(args, tuple) and all(
        isinstance(arg, BExpr) for arg in args)
