...
class Condition:
    ...
    def __init__(self, expr, pos, path=[], blabel=None, post_blabel=None, bottom_ast=None, categ=None, isVC=False, top_asts=[], isPre=False):
        assert isinstance(pos, list)
        # The expression stating the condition itself
        self.expr = expr
        # The positions where the verification condition originates from
        self.pos = pos
        # The positions where the condition originates from
        self.path = path
        # The bottom-most assertion assocation with the condition
        self.bottom_ast = bottom_ast
        ...
        self.isVC = isVC
        # The top assertions associated with the verification condition
        self.top_asts = top_asts 
        ...
        # Whether the condition is pre-condtion of the whole hp
        self.isPre = isPre
...
class AstLoc:
    """Location of assertion(pre-condition, post-condition, invariant)"""
    def __init__(self, index, isPre=False, isPost=False, hp_pos=None):
        # The index of the assertion
        self.index = index
        # Whether the assertion is pre-condition of the whole program
        self.isPre = isPre
        # Whether the assertion is post-condition of the whole program
        self.isPost = isPost
        # Only required if the assertion is an invariant.
...
class CmdVerifier:
    ...
    def __init__(self, *, pre, hp, post, functions=dict()):
        ...
        self.infos[root_pos].post = [Condition(expr=subpost.expr, 
                                               pos=[root_pos], 
                                               bottom_ast=AstLoc(index=idx, isPost=True)
                                               ) 
        ...
        pre_list = expr.split_conj(pre_conj)
        ...
        if isinstance(cur_hp, hcsp.Skip):
            ...
            if not self.infos[pos].inv:
                ...
                for i, sub_inv in enumerate(cur_hp.inv):
                    ...
                    if sub_pos not in self.infos:
                        self.infos[sub_pos] = CmdInfo()
                    sub_info = self.infos[sub_pos]
                    sub_info.inv = [Condition(expr=sub_inv.expr, pos=[pos], bottom_ast=AstLoc(index=i, hp_pos=pos[0]))]
                    sub_info.assume += self.infos[pos].assume
                ...
                for i, subpost in enumerate(post):
                    self.infos[pos].vcs.append(
                        Condition(expr=expr.imp(expr.list_conj(*all_invs), subpost.expr), 
                                            ...
                                            top_asts=[AstLoc(index=i, hp_pos=pos[0]) for i in range(len(cur_hp.inv))],
                                            bottom_ast=subpost.bottom_ast,
                                            categ=subpost.categ,
                                            isVC=True
                                            ))

                pre = [Condition(expr=sub_inv.expr, pos=[pos], bottom_ast=AstLoc(index=i, hp_pos=pos[0]), categ="init") \
                       for i, sub_inv in enumerate(cur_hp.inv)]
            ...
            # For each branches, we compute wp of loop body and verify that the sub_inv is maintained.
                ...
                for sub_pre in body_pre:
                    # bottom_ast add one more tuple to pos to record the annotation index, i.e. invariant index for loop.
        ...
        # Only Cut invariant
            ...
            # TODO: also run if no invariants are specified? testVerify62 testVerify54 testVerify53 testVerify52 testVerify50 testVerify55
                ...
                for i, inv in enumerate(cur_hp.inv):
                    ...
                    self.infos[sub_pos].assume = self.infos[pos].assume + inv_exprs[:i]
                    self.infos[sub_pos].eqs_dict = self.infos[pos].eqs_dict
                    self.infos[sub_pos].inv = Condition(expr=inv.expr, pos=[pos], bottom_ast=i)
                    self.infos[sub_pos].post = [Condition(expr=inv.expr, pos=[pos], bottom_ast=i)]
            ...
            # Assume y(.) solve the symbolic initial value problem y'(t) = f(y), y(0) = x
                ...
                pre = [Condition(expr=expr.ForAllExpr(time_var.name,
                                ...
                                pos=[pos],
                                categ="maintain",
                                bottom_ast=self.infos[pos].inv.bottom_ast)]                                         
            ...
            # Cases when dbx_inv is e >(>=) 0.
                ...
                #    e >(>=) 0 <x_dot = f(x) & D> e >(>=) 0
                    ...
                    if self.infos[pos].dbx_cofactor is None:
                        ...
                        self.infos[pos].vcs.append(Condition(expr=vc, pos=[pos], categ="maintain",
                        bottom_ast=bottom_ast, isVC=True))
                    ...
                    # Cases when the cofactor g is offered by the user.
                        ...
                        self.infos[pos].vcs.append(Condition(expr=vc, pos=[pos], categ="maintain", bottom_ast=bottom_ast, isVC=True))              
            ...
            for i, vc in enumerate(pre):
                self.infos[pos].vcs.append(
                    Condition(expr=expr.imp(pre_conj, vc.expr), 
                              pos=vc.pos,
                              blabel=vc.blabel, 
                              top_asts=[AstLoc(index=i, isPre=True) for i in range(len(self.infos[pos].pre))],
    ...
    def set_solver(self, vc):
        ...
        isPost = vc.bottom_ast.isPost

        # Proof method of each vc is stored in the bottom-most assertion related.

        if isPost:
            assertion = self.post[vc.bottom_ast.index]
        
        else:
            assert vc.bottom_ast.hp_pos is not None
            bottom_hp = get_pos(self.hp, vc.bottom_ast.hp_pos)

            assert isinstance(bottom_hp, (hcsp.Loop, hcsp.ODE))
            assertion = bottom_hp.inv[vc.bottom_ast.index]
