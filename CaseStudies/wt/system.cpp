...
double *k1 = ode1(currentX);
for (int j=0; j<1; j++){
arguments[j] = currentX[j] + k1[j] / 2 * 0;
phi[j] = k1[j] / 6;
}
double *k2 = ode1(arguments);
for (int j=0; j<1; j++){
arguments[j] = currentX[j] + k2[j] / 2 * 0;
phi[j] += k2[j] / 3;
}
double *k3 = ode1(arguments);
for (int j=0; j<1; j++){
arguments[j] = currentX[j] + k3[j] * 0;
phi[j] += k3[j] / 3;
}
double *k4 = ode1(arguments);
for (int j=0; j<1; j++){
...
plant_1=arguments[0];
temp[0]=(control_1_0*(0.8))-(0.0314)*(sqrt((19.6)*plant_1));
return temp;
}
};
SC_MODULE(test) {
