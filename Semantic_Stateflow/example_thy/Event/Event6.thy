...
definition Chart_A_A1 :: state where " Chart_A_A1 = State [''A'', ''A1'']
  ...
  (SKIP)
  []
  [Trans (P [''A'', ''A1'']) (S []) (Bc True) (SKIP) (send1 ''E'' True) (P [''A'', ''A2''])]
  (No_Composition)"
...
definition Chart_B_B1 :: state where " Chart_B_B1 = State [''B'', ''B1'']
  ...
  (SKIP)
  []
  [Trans (P [''B'', ''B1'']) (S [''E'']) ((V ''x'') [>] (N 0)) ((''x'' ::= Minus (V ''x'') (N 1)); ((print1 ''a'' ); (send1 ''E'' False))) (SKIP) (P [''B'', ''B2''])]
  (No_Composition)"
...
definition Chart_B :: state where " Chart_B = State [''B'']
  ...
  []
  []
  (Or ([Trans (NONE) (S []) (Bc True) (''x'' ::= N 6) (SKIP) (P [''B'', ''B1''])])
 (False) (f_Chart_B))"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''a'', ''a'', ''a'', ''a'', ''a'', ''a'', ''en_A2''], ?o2)) (?I))"
