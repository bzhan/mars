...
def run_test(self, filename, num_cycle, res, *,
    ...
    _, _, charts, _, _, _, DSMS, _ = diagram.seperate_diagram()
#     chart = charts[0]
    procs=dict()
    hp_lists=list()
    procs_list=list()
    ...
    if len(charts) >=1:
        for chart in charts:
            converter = sf_convert.SFConvert(chart,DSMS, chart_parameters=diagram.chart_parameters[chart.name])
...
#     procs.update({"init_global_vars":converter.get_global_vars()})
    procs_list.append({"init_global_vars":converter.get_global_vars()})
...
#                 procs.update(proc)
                hp=converter.get_toplevel_process()
                DSM_hp=converter.get_glable_proc()
                print(pprint(hp))  
                for name, proc in proc.items():
                    print('\n' + name + " ::=\n" + pprint(proc))
        print(pprint(DSM_proc))  
        for name, proc in DSM_hp.items():
            print('\n' + name + " ::=\n" + pprint(proc))
...
#     run_simulator_test(self, [(procs, hp)], num_cycle, res)
    run_simulator_test(self,[(procs_list[2],DSM_proc),(procs_list[0], hp_lists[0]),(procs_list[1], hp_lists[1]) ], num_cycle, res)
...
#         run_test(self, "./Examples/Stateflow/tests/community_charts.xml",10,
#            ['log en_add', 'log en_add', 'delay 0.1'],print_before_simp=True)
    def testDsmExample(self):
        run_test(self, "./Examples/Stateflow/tests/DSM_example.xml",4,
