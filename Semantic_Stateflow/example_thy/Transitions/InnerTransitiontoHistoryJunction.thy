theory InnerTransitiontoHistoryJunction
  imports Final_ML
begin
...
definition Chart_A :: state where " Chart_A = State [''A'']
  ...
  (print1 ''During A'' )
  (print1 ''Exit A'' )
  [Trans (P [''A'']) (S [''E_one'']) (Bc True) (SKIP) (SKIP) (HJ [''A''])]
  []
...
schematic_goal "Root_Exec_for_times env ''E_one'' (2::int) s ?s"
  unfolding Chart_A_A1_def Chart_A_A2_def f_Chart_A_def Chart_A_def 
f_Chart_def Root_def g_def v_def I_def fe_def 
