...
definition Chart_S :: state where " Chart_S = State [''S'']
  ...
  (print1 ''duS'' )
  (SKIP)
  [Trans (P [''S'']) (S []) (Bc True) (print1 ''condInner'' ) (print1 ''tranInner'' ) (P [''S'', ''A''])]
  []
  (Or ([Trans (NONE) (S []) (Bc True) (print1 ''condDefault'' ) (print1 ''tranDefault'' ) (P [''S'', ''A''])])
 (False) (f_Chart_S))"
...
schematic_goal "Root_Exec_for_times env ['''', ''''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''enS'', ''condDefault'', ''tranDefault'', ''enA'', ''duS'', ''condInner'', ''exA'', ''tranInner'', ''enA''], ?o2)) (?I))"
