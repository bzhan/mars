theory ConditionTransitionAction
  imports "../Final_ML" 
begin
...
definition Chart_A :: state where " Chart_A = State [''A'']
  ...
  (print1 ''Exit A'' )
  []
  [Trans (P [''A'']) (S [''E_one'']) ((V ''x'') [>] (N (-1))) (print1 ''Condition Action'' ) (print1 ''Transition Action'' ) (P [''B''])]
  (No_Composition)"
...
definition Chart_B :: state where " Chart_B = State [''B'']
  ...
  (print1 ''Exit B'' )
  []
  [Trans (P [''B'']) (S [''E_two'']) (Bc True) (SKIP) (SKIP) (P [''A''])]
  (No_Composition)"
...
schematic_goal "Root_Exec_for_times env [''E_one'', ''E_one''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''Entry A'', ''Condition Action'', ''Exit A'', ''Transition Action'', ''Entry B''], ?o2)) (?I))"
