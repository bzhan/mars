...
definition Chart_A_A1 :: state where " Chart_A_A1 = State [''A'', ''A1'']
  ...
  (print1 ''Exit A1'' )
  []
  [Trans (P [''A'', ''A1'']) (S [''F'']) (Bc True) (SKIP) (SKIP) (P [''A'', ''A2''])]
  (No_Composition)"
...
definition Chart_A :: state where " Chart_A = State [''A'']
  (print1 ''Entry A'' )
  (print1 ''During A'' )
  ((print1 ''Exit A'' );send1 ''F'' False)
  []
  [Trans (P [''A'']) (T1 (Before (''tick'') (N 3))) (Bc True) (SKIP) (SKIP) (P [''B''])]
...
schematic_goal "Root_Exec_for_times env [''E'', ''E''] (2::int) s
 (Status (Vals ?v1 ?v2 ?v3 ?v4 ([''Entry A'', ''Entry A1'', ''Exit A1'', ''Exit A'', ''Exit A'', ''During A'', ''Entry B''], ?o2)) (?I))"
