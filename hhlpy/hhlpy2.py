...
class Predicate:
    ...
    def __init__(self, expr, pos, seq_labels=[], nest_label=None, annot_pos=None, categ=None):
        assert isinstance(pos, list)
        # The expression stating the condition itself
        self.expr = expr
        # The positions where the verification condition originates from
        self.pos = pos
        # Labels for sequence program, stored in a list, in the order from bottom to top.
        self.seq_labels = seq_labels
        # Label to be nested or the final branch label.
        self.nest_label = nest_label
...
class CmdVerifier:
    ...
    def compute_label(self, type, pos, sub_pre=None, index=None, i_num=None):
        """Compute the seq_labels and nest_label for the pre-condition.
            type: the current hcsp program type
            pos: the current position
            sub_pre: the pre-condition of sub-hcsp program
            index: index is the executing branch index, starting from 0, 
                    only necessary in IChoice and ITE
            i_num: the number of IChoice and ITE in the sub-programs of Sequence program
                    only necessary in Sequence
        Return a tuple (seq_labels, nest_label)"""
        ...
        if type == 'ichoice' or type == 'ite':
            ...
            # Compute the new label with the branch index and nest_label.
            # If there is label to be nested, nest it.
            if nest_label:
                new_label = label.NestLabel(value=index+1, sub_label=nest_label)
            else:
                new_label = label.AtomLabel(value=index+1) # value starting from 1

            # Store the new label.
            # If the parent program is Sequence, 
            # append the new label to the seq_labels list, no label to be nested.
            if self.infos[pos].parent_type == 'sequence':
                seq_labels.append(new_label)
                nest_label = None
            # Else if the parent program is IChoice or ITE, set the nest_label as new_label.
            # Or if the parent program is Loop or None, the new label is the complete label tree,
            # store the new label in nest_label.
            elif self.infos[pos].parent_type == 'ichoice' or self.infos[pos].parent_type == 'ite'\
                or self.infos[pos].parent_type == 'loop' or self.infos[pos].parent_type is None:
                nest_label = new_label
            else:
                raise NotImplementedError

            return (seq_labels, nest_label)
        ...
        # When the hcsp program is Sequence
            ...
            # sub_labels is a list, storing the label of each sub-program of Sequence,
            # in order of from top to bottom.
            sub_labels = seq_labels[-i_num:]  # the last i_num labels
            ...
            # If the parent program is IChoice or ITE, set the nest_label as new_label.
            # If the parent program is Loop or None, the new label is the complete label tree,
            # store the new label in nest_label.
            if self.infos[pos].parent_type == 'ichoice' or self.infos[pos].parent_type == 'ite'\
                or self.infos[pos].parent_type == 'loop' or self.infos[pos].parent_type is None:
                nest_label = new_label
            else:
                raise NotImplementedError
        ...
        if isinstance(cur_hp, hcsp.Skip):
            ...
            for i in range(len(cur_hp.hps)):
                ...
                pre += [Predicate(
                    expr=vc.expr, 
                    pos=vc.pos, 
                    seq_labels=self.compute_label(type=cur_hp.type, pos=pos, sub_pre=vc, index=i)[0], 
                    nest_label=self.compute_label(type=cur_hp.type, pos=pos, sub_pre=vc, index=i)[1], 
                    annot_pos=vc.annot_pos, 
                    categ=vc.categ) 
            ...
            pre = [Predicate(vc.expr, 
                    vc.pos + [pos], 
                    seq_labels=self.compute_label(type=cur_hp.type, pos=pos, sub_pre=vc, i_num=i_num)[0],
                    nest_label=self.compute_label(type=cur_hp.type, pos=pos, sub_pre=vc, i_num=i_num)[1],
                    annot_pos=vc.annot_pos, 
                ...
                pre_dw = [Predicate(expr=expr.imp(constraint, subposts[-1]), 
                        ...
                        + \
                         [Predicate(expr=expr.imp(expr.neg_expr(constraint), 
                                                     subpost.expr),
                                                pos=subpost.pos + [pos],
                                                seq_labels=subpost.seq_labels,
                                                nest_label=subpost.nest_label,
                                                categ=subpost.categ)
                          for subpost in post]
            ...
            # When dI_inv is set or by default
                ...
                self.infos[pos].vcs.append(Predicate(expr=vc, pos=[pos], categ="maintain"))

                if dI_inv != post_conj:
                    self.infos[pos].vcs.append(
                        Predicate(expr=expr.imp(dI_inv, post_conj), pos=[pos]))
            ...
            #                   {I} <x_dot = f(x) & D> {I}
                ...
                if dG_inv != post_conj:
                    self.infos[pos].vcs.append(
                        Predicate(expr=expr.imp(dG_inv, post_conj), pos=[pos]))
            ...
            # Cases when dbx_inv is e >(>=) 0.
                ...
                if self.infos[pos].dbx_inv is None:
                    ...
                    self.infos[pos].vcs.append(
                        Predicate(expr=expr.imp(self.infos[pos].dbx_inv, post_conj), pos=[pos]))
            ...
            #      {e >=(>) 0} <x_dot = f(x) & D> {e >=(>) 0}
                ...
                if self.infos[pos].barrier_inv is None:
                    ...
                    self.infos[pos].vcs.append(
                        Predicate(expr=expr.imp(self.infos[pos].barrier_inv, post_conj), pos=[pos]))
