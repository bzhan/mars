...
class Json2HCSP3Test(unittest.TestCase):
    def testJson2HCSP3(self):
        ...
        with open('./Examples/AADL/CCS/HCSP/other_modules.txt', 'w') as f:
            f.write("%type: module\n\n")
            for mod in mods:
                f.write(mod.export())
                f.write('\n\n')

        with open('./Examples/AADL/CCS/HCSP/DataBuffer.txt', 'w') as f:
            f.write("%type: module\n\n")
            for dataBuffer in dataBuffers.values():
                f.write(dataBuffer.export())
                f.write('\n\n')

        with open('./Examples/AADL/CCS/HCSP/Bus.txt', 'w') as f:
            f.write("%type: module\n\n")
            for bus in buses:
                f.write(bus.export())
                f.write('\n\n')

        with open('./Examples/AADL/CCS/HCSP/system.txt', 'w') as f:
            f.write("%type: module\n")
...
if __name__ == "__main__":
    json_file = "./Examples/AADL/CCS/AADL/model_1bus.json"
    unittest.main()
