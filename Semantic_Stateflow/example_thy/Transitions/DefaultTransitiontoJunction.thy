...
definition Chart_B :: state where " Chart_B = State [''B'']
  ...
  []
  []
  (Or ([Trans (NONE) (S []) (Bc True) (SKIP) (SKIP) (J [''B'', ''8''])])
 (False) (f_Chart_B))"
...
" g = (λ str. if str = [''B'', ''8''] then [Trans (J [''B'', ''8'']) (S []) ((V ''x'') [>] (N 0)) (SKIP) (SKIP) (P [''B'', ''B2'']),
Trans (J [''B'', ''8'']) (S []) (Bc True) (SKIP) (SKIP) (P [''B'', ''B1''])] else 
[])"
...
schematic_goal "Root_Exec_for_times env ''E_one'' (2::int) s ?s"
  unfolding Chart_B_B2_def Chart_B_B1_def f_Chart_B_def Chart_B_def Chart_A_def f_Chart_def 
