...
interpret loc:ll_on_open_it "{-1<..}"
  ...
  have s1:" (f solves_ode (\<lambda> t v . g t * v)) {0..T} UNIV" 
    unfolding solves_ode_def
    using assms    
    using has_vderiv_on_subset by blast
 have s2: "(loc.flow 0 0) t = (\<lambda>t. f t) t" if "t \<in> {0..T}" for t
   apply (rule loc.maximal_existence_flow(2)[OF s1])
   using that assms by auto
  have s3:" ((\<lambda> t . 0) solves_ode (\<lambda> t v . g t * v)) {0..T} UNIV"
 unfolding solves_ode_def
  apply auto
  by (simp add: has_vderiv_on_const)
 have s4: "(loc.flow 0 0) t = (\<lambda>t. 0) t" if "t \<in> {0..T}" for t
   apply (rule loc.maximal_existence_flow(2)[OF s3])
  using that assms by auto
