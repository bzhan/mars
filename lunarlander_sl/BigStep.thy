...
fun index_list :: "nat \<Rightarrow> nat list" where
  "index_list 0 = []"
...
| "i < length blkss \<Longrightarrow>
   ...
   combine_blocks sts blkss ((ParWaitBlock 
                                  t
                                 (\<lambda> d. (if (0 \<le> d \<and> d \<le> t) then (wait_block_state_list sts d blkss)
                                                       else undefined)))
                            # pblks)"
...
theorem Valid_skip:
  "Valid
    (\<lambda>t. t = tr)
    (Skip)
    (\<lambda>t. t = extend_trace tr (TauBlock (end_of_trace tr)))"
...
theorem Valid_skip2:
  "Q (extend_trace tr (TauBlock (end_of_trace tr))) \<Longrightarrow>
   Valid
    (\<lambda>t. t = tr)
    (Skip)
    Q"
...
proof-
    have main: "restrict p2 {0..d2} = restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v}\<and> d2 = 1-v"
    if cond:"0 \<le> d2"
       "ODEsol (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1))) p2 d2"
       "\<forall>t. 0 \<le> t \<and> t < d2 \<longrightarrow> p2 t X < 1"
       "\<not> p2 d2 X < 1"
  ...
  proof-
   ...
   have step1:"((loc.flow 0 (state2vec((\<lambda>_. 0)(X := v)))) usolves_ode (\<lambda>t. \<lambda>v. ODE2Vec (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1))) (vec2state v)) from 0) (loc.existence_ivl 0 (state2vec((\<lambda>_. 0)(X := v)))) UNIV"
     using loc.flow_usolves_ode[of "(state2vec((\<lambda>_. 0)(X := v)))"]
     by auto
   have step2:"((\<lambda>t. state2vec((\<lambda>_. 0)(X := v+t))) solves_ode ((\<lambda>t. \<lambda>v. ODE2Vec (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1)))(vec2state v)))) {0..} UNIV"
     ...
     subgoal premises pre for x
     proof-
       have 1:"\<forall>i.((\<chi> x. if x = X then v+y else 0) - (\<chi> xa. if xa = X then v+x else 0) -
            (y - x) *\<^sub>R
            (\<chi> xa. if xa = X
                   then (if xa = X then \<lambda>_. 1 else (\<lambda>_. 0)) (\<lambda>xa. if xa = X then v+x else 0)
                   else 0) ) $ i= 0 " for y 
         by auto
       ...
       have 3:"norm
            ((\<chi> x. if x = X then v+y else 0) - (\<chi> xa. if xa = X then v+x else 0) -
              (y - x) *\<^sub>R
              (\<chi> xa. if xa = X
                    then (if xa = X then \<lambda>_. 1 else (\<lambda>_. 0)) (\<lambda>xa. if xa = X then v+x else 0)
                     else 0)) = 0" for y
         using 1[of y]  2[of "((\<chi> x. if x = X then v+y else 0) - (\<chi> xa. if xa = X then v+x else 0) -
          (y - x) *\<^sub>R
          (\<chi> xa. if xa = X
                  then (if xa = X then \<lambda>_. 1 else (\<lambda>_. 0)) (\<lambda>xa. if xa = X then v+x else 0)
                  else 0))"]
   ...
   have step3:"{0..} \<subseteq> loc.existence_ivl 0 (state2vec((\<lambda>_. 0)(X :=v)))"
     apply(rule loc.existence_ivl_maximal_interval[of "(\<lambda>t. state2vec((\<lambda>_. 0)(X :=v+t)))"])
     using step2 unfolding state2vec_def d1 by auto
   have step4:"t\<in>{0..} \<Longrightarrow>(\<lambda>t. state2vec((\<lambda>_. 0)(X := v+t))) t = (loc.flow 0 (state2vec((\<lambda>_. 0)(X :=v)))) t" for t 
     subgoal premises pre 
     apply (rule usolves_odeD(4)[of "loc.flow 0 (state2vec((\<lambda>_. 0)(X :=v)))" "(\<lambda>t v. ODE2Vec (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1))) (vec2state v))" "0" "(loc.existence_ivl 0 (state2vec ((\<lambda>_. 0)(X:=v))))" "UNIV" "{0..}" " (\<lambda>t. state2vec((\<lambda>_. 0)(X := v+t)))" t])
       ...
       subgoal  
       proof-
         have "loc.flow 0 (state2vec ((\<lambda>_. 0)(X := v))) 0  = (state2vec ((\<lambda>_. 0)(X := v)))"
           using loc.flow_initial_time [of "(state2vec ((\<lambda>_. 0)(X := v)))"] by auto
         then show ?thesis unfolding fun_upd_def state2vec_def by auto
       qed
       done
     done
   have step5:"((\<lambda>t. state2vec(p2 t)) solves_ode ((\<lambda>t. \<lambda>v. ODE2Vec (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1)))(vec2state v)))) {0..d2} UNIV"
     using cond(2) unfolding ODEsol_def solves_ode_def by auto
   have step6:"{0..d2} \<subseteq> loc.existence_ivl 0 (state2vec((\<lambda>_. 0)(X:=v)))"
   ...
   have step7:"t\<in>{0..d2} \<Longrightarrow>(\<lambda>t. state2vec (p2 t)) t = (loc.flow 0 (state2vec((\<lambda>_. 0)(X:=v)))) t" for t 
     subgoal premises pre 
     apply (rule usolves_odeD(4)[of "loc.flow 0 (state2vec ((\<lambda>_. 0)(X:=v)))" "(\<lambda>t v. ODE2Vec (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1))) (vec2state v))" "0" "(loc.existence_ivl 0 (state2vec ((\<lambda>_. 0)(X:=v))))" "UNIV" "{0..d2}" " (\<lambda>t. state2vec (p2 t))" t])
       subgoal using step1 by auto
   ...
   have step8:"1-v \<le> d2" 
    proof(rule ccontr)
     assume 0:" \<not> (1-v \<le> d2)"
     from 0 have 1:"(\<lambda>t. state2vec((\<lambda>_. 0)(X := v+t))) d2 = (\<lambda>t. state2vec(p2 t)) d2"
       using step4[of d2] step7[of d2] cond(1) by auto 
     from 1 have 2:"((\<lambda>_. 0)(X := v+d2)) = p2 d2"
       unfolding state2vec_def by auto
     from 2 have 3:"p2 d2 X < 1" using 0 
     ...
     then show "False" using 3 by auto
   qed
   have step9:"1-v \<ge> d2"
   proof(rule ccontr)
     assume 0:"\<not> d2 \<le> 1-v" 
     from 0 have 1:"(\<lambda>t. state2vec((\<lambda>_. 0)(X := v+t))) (1-v) = (\<lambda>t. state2vec(p2 t)) (1-v)"
       using step4[of "1-v"] step7[of "1-v"] cond(1) d1 by auto
     from 1 have 2:"((\<lambda>_. 0)(X := 1)) = p2 (1-v)"
       unfolding state2vec_def by auto
     have 3:"p2 (1-v) X < 1" using cond 0 d1 by auto
     have 4:"p2 (1-v) X = 1" using 2 unfolding fun_upd_def
       by (metis "2" fun_upd_same)
     show "False" using 3 and 4 by auto
   qed
   have step10:"d2 = 1-v" using step8 step9 by auto
   have step11:"t\<in>{0..1-v} \<Longrightarrow> (p2 t) =  ((\<lambda>t. (\<lambda>_. 0)(X := t+v)) t)" for t
     using step4 step7 step10
     by (smt atLeastAtMost_iff atLeast_iff vec_state_map1)
   have step12:"restrict p2 {0..d2} = restrict (\<lambda>t. (\<lambda>_. 0)(X := t+v)) {0..1-v}"
     using step10 step11 unfolding restrict_def by auto
    show ?thesis using step10 step12 by auto
  qed
  have main1: "restrict p2 {0..d2} = restrict(\<lambda>t. (\<lambda>_. 0)(X := v+t)){0..1-v}\<and> d2 = 1-v"
    if cond:"0 \<le> d2"
       "ODEsol (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1))) p2 d2"
       "\<forall>t. 0 \<le> t \<and> t < d2 \<longrightarrow> p2 t X < 1"
       "\<not> p2 d2 X < 1"
       "p2 0 = end_of_trace (Trace (\<lambda>_. 0) list)"
     for p2 d2
  proof-
    have "p2 0 = (\<lambda>_. 0)(X:=v)" using cond d2 by auto
    then show ?thesis using main[of d2 p2] cond
      by (smt restrict_ext)
  qed
  show ?thesis
    apply(rule Valid_ode_solution2[where d="1-v" and p="\<lambda>t. (\<lambda>_. 0)(X := v+t)"])
...
fun left_blocks :: "real \<Rightarrow> (time \<times> real \<times> time) list \<Rightarrow> trace_block list" where
  "left_blocks x [] = []"
| "left_blocks x  ((dly1, v, dly2) # rest) = (if x<1 then
      ODEBlock (1-x) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+x)){0..1-x}) #
      OutBlock dly1 ''ch'' 1 ({''ch''}, {}) # 
      InBlock dly2 ''ch'' X v ({}, {''ch''}) #left_blocks v rest 
else  ODEBlock 0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := x)){0..0}) #
...
lemma left_blocks_snoc: 
  assumes "length dlys > 0"
    ...
    shows "left_blocks n (dlys @ [(dly1, v, dly2)]) = left_blocks n dlys @ (if x<1 then
      [ODEBlock (1-x) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+x)){0..1-x}),
      OutBlock dly1 ''ch'' 1 ({''ch''}, {}), 
...
lemma end_left_blocks:
  "end_of_blocks ((\<lambda>_. 0)(X := v)) (left_blocks v dlys) = (if length dlys = 0 then (\<lambda>_. 0)(X := v) else (\<lambda>_. 0)(X := fst (snd (last dlys))))" 
  apply (induct dlys arbitrary: v)
   by auto

lemma end_left_blocks_init:
  "end_of_blocks (\<lambda>_. 0) (left_blocks 0 dlys) = (if length dlys = 0 then (\<lambda>_. 0)(X := 0) else (\<lambda>_. 0)(X := fst (snd (last dlys))))"
proof -
  have 1: "(\<lambda>_. 0) = (\<lambda>_. 0)(X := 0)"
    by auto
  show ?thesis
...
lemma end_left_blocks_1:
  "end_of_blocks ((\<lambda>_. 0)(X := v)) ((left_blocks v dlys)@[ODEBlock d (restrict p {0..d})]) = restrict p {0..d} d" 
  apply (induct dlys arbitrary: v)
  by auto

lemma end_left_blocks_1_init:
  "end_of_blocks ((\<lambda>_. 0)) ((left_blocks 0 dlys)@[ODEBlock d (restrict p {0..d})]) = restrict p {0..d} d" 
proof-  
  have 1: "(\<lambda>_. 0) = (\<lambda>_. 0)(X := 0)"
    by auto
  show ?thesis
...
proof -
  have main:"Valid
    ...
    proof cases
      ...
      have 1:"(left_blocks 0 x) = []"
        using end_left_blocks_init c1 by auto
      have 2:"Valid
    (\<lambda>t. t = Trace (\<lambda>_. 0) (left_blocks 0 x))
    (Cont (ODE {X} ((\<lambda>_. \<lambda>_. 0)(X := (\<lambda>_. 1)))) (\<lambda>s. s X < 1))
    (\<lambda>t. t = Trace (\<lambda>_. 0) [ODEBlock 1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t)){0..1})])"
     ...
     (\<lambda>t. t = Trace (\<lambda>_. 0) [ODEBlock 1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t)){0..1})])
      ...
      (\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) [ODEBlock 1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t)){0..1}),OutBlock dly1 ''ch'' 1 ({''ch''}, {})])"
        apply (rule Valid_send2)
      ...
      show ?thesis 
        apply(rule Valid_seq[where Q="(\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) [ODEBlock 1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t)){0..1}),OutBlock dly1 ''ch'' 1 ({''ch''}, {})])"])
         defer 
        subgoal 
          using 4 by auto
        subgoal
    ...
    next
      ...
      have 1:"end_of_trace (Trace (\<lambda>_. 0) (left_blocks 0 x)) = (\<lambda>_. 0)(X := v)"
        using end_left_blocks_init c2 ini by auto
      have 2:"Valid
    (\<lambda>t. t = Trace (\<lambda>_. 0) (left_blocks 0 x))
    (Cont (ODE {X} ((\<lambda>_. \<lambda>_. 0)(X := (\<lambda>_. 1)))) (\<lambda>s. s X < 1))
    (\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  (1-v) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v})]))" 
        if cond1:"v<1"
        using testHL12' cond1 ini end_left_blocks_init c2 by auto
      have 3:"Valid
    (\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  (1-v) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v})]))
    (Cm (Send ''ch'' (\<lambda>s. s X )))
    (\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  (1-v) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v}),OutBlock dly1 ''ch'' 1 ({''ch''}, {})]))"
        if cond1:"v<1"
        apply(rule Valid_send2)
        apply auto
        subgoal for dly
      ...
      (\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (left_blocks 0 dlyvs))" 
        ...
        subgoal for dly1
          ...
          apply auto
          subgoal for dly va 
            apply(rule exI[where x= "x@[(dly1,va,dly)]"])
            using c2 ini cond1 left_blocks_snoc[of x v 0 dly1 va dly]
            unfolding extend_receive_def by auto
          done
        done
      have 5:" Valid (\<lambda>tr. tr = Trace (\<lambda>_. 0) (left_blocks 0 x))
     ((Cont (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1)))
        (\<lambda>s. s X < 1); Cm (''ch''[!](\<lambda>s. s X))); Cm (''ch''[?]X))
     (\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (left_blocks 0 dlyvs))" 
        if cond1:"v<1"
        apply(rule Valid_seq[where Q="(\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  (1-v) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v}),OutBlock dly1 ''ch'' 1 ({''ch''}, {})]))"])
         defer
        subgoal using 4 cond1 by auto
        apply(rule Valid_seq[where Q="(\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  (1-v) (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..1-v})]))"])
        using 2 3 cond1 by auto
     have 6:"Valid
    (\<lambda>t. t = Trace (\<lambda>_. 0) (left_blocks 0 x))
    (Cont (ODE {X} ((\<lambda>_. \<lambda>_. 0)(X := (\<lambda>_. 1)))) (\<lambda>s. s X < 1))
    (\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0})]))" 
       if cond2:"v\<ge>1"
    ...
    (\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0})]))
    (Cm (Send ''ch'' (\<lambda>s. s X )))
    (\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0}),OutBlock dly1 ''ch'' v ({''ch''}, {})]))"
       if cond2:"v\<ge>1"
       apply(rule Valid_send2)
       apply auto
       subgoal for dly
         apply(rule exI[where x="dly"])
         unfolding extend_send_def apply auto
         using cond2 end_left_blocks_1_init[of x 0 "(\<lambda>t\<in>{0}. (\<lambda>_. 0)(X := v))"] by auto
       done
      have 8:"Valid
      (\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0}),OutBlock dly1 ''ch'' v ({''ch''}, {})]))
      (Cm (Receive ''ch'' X))
      (\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (left_blocks 0 dlyvs))" 
        ...
        subgoal for dly1
          ...
          subgoal for dly va 
            apply(rule exI[where x= "x@[(dly1,va,dly)]"])
            using c2 ini cond2 left_blocks_snoc[of x v 0 dly1 va dly]
            unfolding extend_receive_def by auto
          done
        done
      have 9:" Valid (\<lambda>tr. tr = Trace (\<lambda>_. 0) (left_blocks 0 x))
     ((Cont (ODE {X} ((\<lambda>_ _. 0)(X := \<lambda>_. 1)))
        (\<lambda>s. s X < 1); Cm (''ch''[!](\<lambda>s. s X))); Cm (''ch''[?]X))
     (\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (left_blocks 0 dlyvs))" 
        if cond2:"v\<ge>1"
        apply(rule Valid_seq[where Q="(\<lambda>t. \<exists>dly1. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0}),OutBlock dly1 ''ch'' v ({''ch''}, {})]))"])
         defer
        subgoal using 8 cond2 by auto
        apply(rule Valid_seq[where Q="(\<lambda>t. t = Trace (\<lambda>_. 0) ((left_blocks 0 x)@[ODEBlock  0 (restrict(\<lambda>t. (\<lambda>_. 0)(X := v)){0..0})]))"])
        using 6 7 cond2 by auto
      show ?thesis
   ...
   then show ?case
    ...
    next
      ...
      5: "(snd (snd b)) = 0" "(snd (snd a)) = 0" "(fst (snd b)) = 0" "combine_blocks [(\<lambda>_. 0)(X := 0), (\<lambda>_. 0)(X := 1)] [left_blocks (fst(snd b)) list,right_blocks dlyvs] rest2"
         ...
         using combine_blocks_IO2' by(force)
      have 51: "(\<lambda>_. 0)(X := 0) = (\<lambda>_. 0)" by auto
      from 51 obtain n where 6: "rest2 = tot_blocks n"
...
fun ileft_blocks :: "real \<Rightarrow> (time \<times> real \<times> time) list \<Rightarrow> trace_block list" where
  "ileft_blocks x [] = []"
| "ileft_blocks x  ((dly1, v, dly2) # rest) = 
      ODEOutBlock dly1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+x)){0..dly1}) ''ch'' (dly1+x) ({''ch''}, {}) # TauBlock ((\<lambda>_. 0)(X := dly1+x)) # 
      InBlock dly2 ''ch'' X v ({}, {''ch''}) #ileft_blocks v rest "


lemma ileft_blocks_snoc: 
  assumes "length dlys > 0"
      and "x = fst(snd(last dlys))"
    shows "ileft_blocks n (dlys @ [(dly1, v, dly2)]) = ileft_blocks n dlys @ 
      [ODEOutBlock dly1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+x)){0..dly1}) ''ch'' (dly1+x) ({''ch''}, {}), TauBlock ((\<lambda>_. 0)(X := dly1+x)),
      InBlock dly2 ''ch'' X v ({}, {''ch''})]"
  using assms
  apply auto
...
lemma testHL14o:
  assumes "end_of_trace (Trace (\<lambda>_. 0) list) = (\<lambda>_. 0)(X := v)"
    ...
    (\<lambda>t. \<exists>d. t = Trace (\<lambda>_. 0) (list@[ODEOutBlock d (restrict(\<lambda>t. (\<lambda>_. 0)(X := t+v)){0..d}) ''ch'' (d+v) ({''ch''}, {}),TauBlock ((\<lambda>_. 0)(X := d+v))]))"
  apply(rule Valid_interrupt)
  apply auto
  subgoal premises pre for p d
  proof-
  ...
  show ?thesis
    ...
    using 1 apply auto
    subgoal unfolding restrict_def using pre 
      by (metis atLeastAtMost_iff fun_upd_same order_refl)
    using pre(1)
    by (simp add: end_of_blocks_append)
qed
  done
    
lemma end_ileft_blocks:
  "end_of_blocks ((\<lambda>_. 0)(X := v)) (ileft_blocks v dlys) = (if length dlys = 0 then (\<lambda>_. 0)(X := v) else (\<lambda>_. 0)(X := fst (snd (last dlys))))" 
  apply (induct dlys arbitrary: v)
   by auto

lemma end_ileft_blocks_init:
  "end_of_blocks (\<lambda>_. 0) (ileft_blocks 0 dlys) = (if length dlys = 0 then (\<lambda>_. 0)(X := 0) else (\<lambda>_. 0)(X := fst (snd (last dlys))))"
proof -
  have 1: "(\<lambda>_. 0) = (\<lambda>_. 0)(X := 0)"
    by auto
  show ?thesis
    apply (subst 1)
    apply (subst end_ileft_blocks)
    by auto
qed
     ...
     (\<lambda>tr. \<exists>dly1. tr = Trace (\<lambda>_. 0) [ODEOutBlock dly1 (restrict(\<lambda>t. (\<lambda>_. 0)(X := t)){0..dly1}) ''ch'' (dly1) ({''ch''}, {}),TauBlock ((\<lambda>_. 0)(X := dly1))])"
      ...
      ((\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (ileft_blocks  0 dlyvs)))"
        ...
        subgoal for x
          ...
          apply auto
          subgoal for dly v
            apply(rule exI[where x = "[(x,v,dly)]"])
            unfolding extend_receive_def
            by auto
          done
        done
    ...
    next
      ...
      (\<lambda>tr. \<exists>dlyvs. tr = Trace (\<lambda>_. 0) (ileft_blocks 0 dlyvs))"
        ...
        subgoal for xa
          ...
          apply auto 
          subgoal for dly va
            apply(rule exI[where x ="x@[(xa,va,dly)]"])
            unfolding extend_receive_def
            using ileft_blocks_snoc[of x v 0 xa va dly] ini c2 by auto
          done
        done
      show ?thesis
