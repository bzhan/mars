...
class HHLPyTest(unittest.TestCase):
    ...
    def nonlinearTestVerify8(self):
        ...
        runVerify(self, pre="x^2 + (2+y)^2 <= 1",
                  ...
                  ode_invariants={((1,), ()): "0.0052726*x^10+0.013182*x^8*y^2+0.013181*x^6*y^4+0.0065909*x^4*y^6+0.0016477*x^2*y^8+0.00016477*y^10-0.060426*x^8*y-0.11666*x^6*y^3-0.08401*x^4*y^5-0.02829*x^2*y^7-0.0026618*y^9-0.0093935*x^8+0.25715*x^6*y^2+0.35556*x^4*y^4+0.18385*x^2*y^6+0.017843*y^8-0.22922*x^6*y-0.82409*x^4*y^3-0.6654*x^2*y^5-0.072582*y^7+0.38533*x^6+1.6909*x^4*y^2+1.7759*x^2*y^4+0.20099*y^6+1.8855*x^4*y-0.83113*x^2*y^3-0.10854*y^5-4.9159*x^4-11.581*x^2*y^2-1.9047*y^4+6.644*x^2*y+7.8358*y^3+1.5029*x^2-13.2338*y^2+10.8962*y-3.4708 <= 0 \
                  && \
                  0.10731*x^10+0.26827*x^8*y^2+0.26827*x^6*y^4+0.13413*x^4*y^6+0.033534*x^2*y^8+0.0033532*y^10-1.2677*x^8*y-2.4914*x^6*y^3-1.8208*x^4*y^5-0.59588*x^2*y^7-0.057773*y^9-0.82207*x^8+4.1107*x^6*y^2+6.7924*x^4*y^4+3.4828*x^2*y^6+0.36938*y^8+6.8306*x^6*y-0.93431*x^4*y^3-5.9328*x^2*y^5-0.95223*y^7+2.2556*x^6-17.4284*x^4*y^2-6.4448*x^2*y^4-0.33741*y^6-1.2936*x^4*y+16.8675*x^2*y^3+8.8828*y^5-16.1915*x^4-39.7751*x^2*y^2-25.8126*y^4+43.7284*x^2*y+39.2116*y^3-12.7866*x^2-33.0675*y^2+15.2878*y-3.1397 <= 0"},
    ...
    def nonlinearTestVerify9(self):
        ...
        runVerify(self, pre="(1+x)^2 + (-2+y)^2 <= 4/25",
                  ...
                  post="~((-1+x)^2 + y^2 <= 1/25)",
                  ode_invariants={((1,), ()): "0.23942*x^6 + 0.097208*x^5*y + 0.06013*x^4*y^2 - 0.0076888*x^3*y^3 - 0.022097*x^2*y^4 + 0.067444*x*y^5 + 0.063249*y^6 - 0.11511*x^5 - 0.093461*x^4*y - 0.061763*x^3*y^2 + 0.065902*x^2*y^3 + 0.053766*x*y^4 - 0.1151*y^5 - 0.95442*x^4 + 0.38703*x^3*y + 0.46309*x^2*y^2 - 0.14691*x*y^3 + 0.11756*y^4 - 0.021196*x^3 - 0.40047*x^2*y - 0.28433*x*y^2 - 0.028468*y^3 - 0.020192*x^2 - 0.37629*x*y - 0.13713*y^2 + 1.9803*x - 1.4121*y - 0.51895 <= 0"},
                  barrier_certificate_rule={((1,), ()): "true"},
                  wolfram_engine=True)
    ...
    # def nonlinearTestVerify11(self):
    ...
    #     runVerify(self, pre="-1/20 + (5/4 + x)^2 + (-5/4 + y)^2 <= 0",
    #               hp="t := 0; <x_dot = x + y, y_dot = x * y - y^2 / 2, t_dot = 1 & t < 10>",
    #               post="~((5/2 + x)^2 + (-4/5 + y)^2 <= 1/20)",
    #               diff_cuts={((1,), ()): ["y > 0", "y*((-104420)+(-73565)*x+18407*y) < 44444"]},
    #               darboux_rule={((1,), (0,)): "true"},
