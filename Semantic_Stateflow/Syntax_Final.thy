...
datatype vals =  Vals "var_val" "event_val" "time_val" "string list \<times> real list"
...
fun aval :: "exp \<Rightarrow> vals \<Rightarrow> path \<Rightarrow> val" where
  "aval (N n) v p = n" |
  "aval (V x) (Vals s h1 h2 (output1, output2)) p = s x" |
  ...
  "aval (TemporalCount x) (Vals s h1 h2 (output1, output2)) p = (if (x = ''Sec'') then (h2 p) 
                                              else (h1 p x))" 
...
datatype event = S "string list" | T1 temp_cond
...
Before: "tval1 (Before E expr) (Vals s h1 h2 (output1, output2)) p 
         = (if (E = ''Sec'') then ((h2 p) < (aval expr (Vals s h1 h2 (output1, output2)) p)) 
         else (real_of_int(h1 p E) < (aval expr (Vals s h1 h2 (output1, output2)) p)))"
| After: "tval1 (After E expr) (Vals s h1 h2 (output1, output2)) p 
         = (if (E = ''Sec'') then ((h2 p) \<ge> (aval expr (Vals s h1 h2 (output1, output2)) p)) 
         else (real_of_int(h1 p E) >= (aval expr (Vals s h1 h2 (output1, output2)) p)))"
| At: "tval1 (At E expr) (Vals s h1 h2 (output1, output2)) p
         = (real_of_int(h1 p E) = (aval expr (Vals s h1 h2 (output1, output2)) p))" 
| Every: "tval1 (Every E expr) (Vals s h1 h2 (output1, output2)) p 
          = (((h1 p E) > 0) \<and> ((aval expr (Vals s h1 h2 (output1, output2)) p) dvd real_of_int(h1 p E)) )"
